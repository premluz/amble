...
## `ZoneGridScreen`'s day-columns shared ONE `rangeStart` across all 7 days, stacking every day after Monday a full 24h too low

**Symptom**: a widget test tapping a zone in a Tuesday column consistently missed, landing on an unrelated render object one full `gridHeight` (1056px) below the expected position. Wednesday's zones would have been two `gridHeight`s off, and so on — confirmed by hand once the exact 1056px discrepancy was measured and matched to `24h * 44px/hour`.

**Cause**: each day-column's `_blockFor` computed a zone's `top` as `zoneStart.difference(rangeStart).inMinutes * pixelsPerMinute`, where `rangeStart` was the WEEK's shared value (Monday's own midnight), passed identically to every column. For Monday's own zones this is correct by coincidence (Monday's midnight IS the reference point). For every later day, `zoneStart` (e.g. Tuesday 13:00) measured against Monday's midnight includes a full extra day (or more) of minutes that don't belong in that column's own local coordinate space — each day-column is its own independent 24-hour axis, not a slice of one continuous week-long axis.

**Fix**: compute `top` against `dayStart` (`DateTime(day.year, day.month, day.day)`, freshly derived per-column from that column's own `day`), not the shared `rangeStart`. The shared `rangeStart` is still correct and still used for exactly one thing: `_HourAxis`'s own tick LABELS, which genuinely are one shared ruler down the left edge of the whole grid.

**How this was found, since it wasn't obvious from the failure alone**: the first three hypotheses (test viewport too short so the second zone scrolled off-screen; `find.text()`'s well-known "warning, not failure" hit-test quirk on non-hit-testable `Text` nodes; a stale animation frame from `runAsync`) were each tried and ruled out in turn — none explained why the SAME test, with a taller viewport and `warnIfMissed: false` added, still measured a real off-screen coordinate for a SECOND zone while a first, Monday-only zone test passed cleanly. Printing the actual `getRect()` of both zones directly in the test (temporarily) surfaced the 1232px gap, and dividing that by the known `pixelsPerHour` constant identified it as almost exactly one `gridHeight` plus the expected small time-of-day delta — at which point the shared-`rangeStart` bug was obvious from reading `_blockFor` a second time.

**Rule**: when several widgets in a per-column/per-item layout each derive geometry from "the start of their own axis," pass each one ITS OWN start value, not one shared reference value computed once outside the loop — even when the shared value happens to be correct for the FIRST item, which is exactly what let this compile, analyze cleanly, and pass a first quick single-zone test before a second zone on a later day exposed it. Add a multi-day (not just multi-zone) fixture to any test suite covering a multi-column time-based layout specifically to catch this class of bug.

## `ZoneGridScreen`'s `_SelectionBar` overflowed by 34px when a recurring zone was selected — only visible on a real device, not caught by widget tests

**Symptom**: on-device screenshot of the selection bar (count + "Affect future instances" label + `AppSwitch` + "Clear") for a recurring-zone selection showed Flutter's debug "RIGHT OVERFLOWED BY 34 PIXELS" banner, rendered as a black/yellow hazard strip directly on top of the switch.

**Cause**: the toggle's `Row` (`mainAxisAlignment: MainAxisAlignment.end`, inside an `Expanded`) laid out the "Affect future instances" `Text` at its full intrinsic width alongside the switch — on a narrow phone width, that plus the leading "N selected" count and the trailing "Clear" text together exceeded the available row width, with nothing set up to shrink.

**How this was missed by the widget test suite**: `zone_grid_screen_test.dart`'s "toggle appears for recurring selection" test only asserts the toggle widgets are present/absent (`find.text`/`find.byType` existence checks) — it never runs a real layout at a real device width, and `flutter test`'s default test surface is wide enough that the overflow doesn't trigger. It only showed up in an actual on-device screenshot at the phone's real screen width.

**Fix**: wrap the "Affect future instances" label in `Flexible` with `maxLines: 1` and `TextOverflow.ellipsis`, so it shrinks to "Affect future ins…" instead of forcing the row wider than its constraint.

**Rule**: a widget test asserting a control's *presence* is not the same as asserting its *layout fits* — RenderFlex overflows are a real-width-only failure mode that automated tests at a generous test-harness width can pass straight through. This is the concrete reason the work order's "when done" checklist required real on-device screenshots in addition to `flutter test`, not instead of it.

## `ZoneGridScreen`'s new `EditModeDeleteTarget` was missing its `Positioned` wrapper, so every drop silently missed it

**Symptom**: a widget test dragged a selected zone onto the visible, correctly-armed (`isArmed == true`) delete target and released — the zone was never deleted. The target rendered, faded in on drag start, and turned solid red on hover exactly as expected; only the actual drop silently did nothing.

**Cause**: `EditModeDeleteTarget(...)` was added directly as a `Stack` child with no `Positioned` wrapper, unlike the Timeline's own placement of the identical widget (`timeline_screen.dart`: `Positioned(left: 0, right: 0, bottom: 0, child: EditModeDeleteTarget(...))`). Without it, the target sized itself to its own intrinsic content and sat at the `Stack`'s default top-left origin instead of being pinned across the full width of the viewport's bottom edge. `tester.getCenter(find.byType(EditModeDeleteTarget))` in the test therefore resolved to a real, on-screen point — just not the one `isInsideDeleteTarget`'s bounds check (resolved independently, from the SAME widget's own `targetKey`) was comparing the drop's `globalPosition` against, since the widget's actual on-screen rect wasn't where a bottom-pinned target's rect was assumed to be everywhere else that reads this code.

**How this was found**: a debug `print` temporarily added to `_onMoveEnd` showed `droppedOnTarget: true` and the delete call genuinely firing — ruling out the drop-detection logic itself — which redirected the search to why the delete's EFFECT wasn't showing up, and from there to a second, unrelated bug (below) before finally re-reading `EditModeDeleteTarget`'s two call sites side by side and noticing the missing wrapper.

**Fix**: wrap it in `Positioned(left: 0, right: 0, bottom: 0, child: EditModeDeleteTarget(...))`, matching the Timeline's own placement exactly.

**Rule**: when reusing a widget that is documented as needing a specific `Positioned`/layout context at its ONE existing call site, copy that placement verbatim rather than re-deriving it — a widget that "renders and looks right" with no wrapper can still be sitting at the wrong on-screen rect for anything that hit-tests against its bounds independently (via a `GlobalKey`, as here) rather than through its own gesture handlers.

## A real async Hive write inside a drag's `onDragEnd` doesn't complete on a fixed-duration `tester.pump()` — and `pumpAndSettle()` hangs forever if a selected block is still animating

**Symptom**: a widget test dragged a zone onto the delete target, called `await gesture.up(); await tester.pump(); await tester.pump(const Duration(milliseconds: 400));`, then asserted the zone's text was gone — and it wasn't. A later attempt swapped the fixed-duration pumps for `await tester.pumpAndSettle()`, expecting it to simply wait longer; the whole test run instead hung for over 6 minutes before failing.

**Cause, first symptom**: `ZoneList.deleteZone`/`deleteZoneSeries` are real `Future`-returning Hive I/O (`box.delete`), called fire-and-forget from `_onMoveEnd` (matching every other `ZoneList` write's own call-site discipline — nothing in this codebase awaits these inline from a gesture callback). `tester.pump(Duration)` only advances Flutter's FAKE animation clock inside the test binding; it does not pump the real event loop that a genuine `Future`-based write resolves on. A `find.text(...)` assertion immediately after can run before that microtask has actually completed.

**Cause, second symptom**: `pumpAndSettle()` repeatedly pumps frames until no more are scheduled. Any zone left selected after the drop still has its own `EditModeWiggle` running an endless (non-terminating) animation — "no more frames scheduled" never becomes true, so `pumpAndSettle` never returns.

**Fix**: this codebase already has the correct pattern for the first problem, in `test/features/timeline/multi_task_group_delete_test.dart` — run the ENTIRE gesture (`startGesture` → `moveBy`/`moveTo` → `up`) inside `tester.runAsync(() async { ... })`, ending with a real `await Future<void>.delayed(const Duration(milliseconds: 100))` INSIDE that same `runAsync` block, followed by one ordinary `await tester.pump(const Duration(milliseconds: 400))` outside it. `runAsync` runs its callback on the real event loop, so the real delay genuinely gives the real I/O time to finish before the final pump renders the result. For the second problem, don't reach for `pumpAndSettle` at all when a wiggling block might still be on screen afterward — a couple of ordinary bounded `pump()` calls (as above) is both correct and fast; reserve `pumpAndSettle` for scenes with no infinite-loop animations live.

**Rule**: a `Future`-returning write fired from inside a widget callback (not awaited by the caller, by design) needs `tester.runAsync` plus a real delay to verify in a test — a fixed-duration `pump()` only fast-forwards fake animation time, never real I/O. And treat `pumpAndSettle()` as unsafe by default in any screen with a permanently-looping animation (a wiggle, a spinner, a shimmer) still in the tree — it will hang rather than error, which is far more expensive to diagnose than a false negative.

## Two new `zone_form_screen_test.dart` tests asserted on `AppSegmentedTimeField`'s value right after `enterText`, before it had actually committed

**Symptom**: a test typed `'0700'` into the Start time field via `tester.enterText` and immediately checked whether Save was disabled — expecting `isNull` (a half-typed start/end pair should stay refused) and getting exactly that, but for the WRONG reason: the value never actually reached `_startHour`/`_startMinute` at all, so the assertion "passed" against a field that still read as fully blank rather than half-filled. A second, structurally identical test (typing both Start and End the same way, then tapping Save to verify a squeeze-not-block save) failed outright, since THAT one needed a real committed value to reach `_save()` in the first place.

**Cause**: `AppSegmentedTimeField` (`lib/core/widgets/app_segmented_time_field.dart`) only calls its own `onChanged` — the callback that actually updates `_startHour`/`_startMinute`/`_endHour`/`_endMinute` in the form's state — from `_commit()`, which fires on the field's focus LEAVING it (`_handleFocusChange`) or on `TextField.onEditingComplete`. A bare `tester.enterText(...)` changes the underlying `TextField`'s text directly without triggering either — the masked-input `TextEditingController` sees new text, but nothing tells `AppSegmentedTimeField` to parse and commit it.

**How this was caught**: a temporary debug `print` inside `_canSave` showed `_startHour`/`_startMinute` still `null` immediately after the `enterText` call that was supposed to have set them — confirming the value genuinely never landed, rather than landing and being read wrong.

**Fix**: explicitly commit the field before reading anything downstream of it — `await tester.testTextInput.receiveAction(TextInputAction.done); await tester.pump();` after the `enterText` call (or tap a second field, which blurs the first — this codebase's own pre-existing "editing start time and tapping Save WITHOUT tapping away first" test already relies on `_save()`'s own explicit unfocus-and-await-a-frame flush for the SAME reason, just from the opposite direction).

**Rule**: a masked/segmented input field that defers its own `onChanged` to blur/submit (rather than firing on every keystroke) needs an explicit commit step in a test — `enterText` alone only proves the RAW TEXT changed, not that the value the rest of the form reads has updated. A test that "passes" on a value that never actually committed is invisible until a second test comes along that actually depends on the committed value working.

## Two `zone_form_screen_test.dart` tests hung indefinitely by reading a pushed route's pop result INSIDE the same `tester.runAsync` block that tapped Save

**Symptom**: two new tests (verifying `showZoneFormScreen`'s new `ZoneFormResult` return value) each hung for 5+ minutes before the whole suite run timed out, with no assertion failure — just "did not complete."

**Cause**: both tests stored `showZoneFormScreen`'s own returned future (`resultFuture`) before interacting with the form, then tried to `return resultFuture;` from inside the SAME `tester.runAsync(() async { ...tap Save...; return resultFuture; })` block used to tap Save and wait for its real (Hive) I/O to settle. `resultFuture` only resolves once `Navigator.pop`'s own pop TRANSITION has actually been pumped through real animation frames — and nothing pumps a frame while `runAsync`'s own callback is still executing (`runAsync` runs its body on the real event loop, deliberately outside the test binding's normal frame-pumping machinery, which is exactly why it's needed for real I/O in the first place). The two goals are incompatible inside one `runAsync` call: real-I/O-settling needs to NOT pump frames; observing the pop needs TO pump frames. The result was neither goal met — `resultFuture` sat forever unresolved, and `await tester.runAsync(...)` waited on it forever.

**How this was caught**: the exact same file already has a WORKING test ("editing the start time and tapping Save WITHOUT tapping away first") that taps Save inside `runAsync`, then calls `pumpAndSettle()` OUTSIDE it afterward, then reads the resulting Hive state directly — it never tries to read a stored `Future` from inside `runAsync`. Comparing the hanging tests against that working one line-by-line (rather than continuing to guess at UI/hit-test causes, which a first-pass reading of the failure log's benign hit-test warnings had misleadingly suggested) surfaced the actual structural difference.

**Fix**: keep `runAsync` scoped to ONLY the tap-and-real-delay step (no return value read from it), call `await tester.pumpAndSettle()` OUTSIDE that block afterward (this is what actually lets the pop transition complete), and only THEN `await resultFuture` — by that point it's already resolved, so the await returns immediately.

**Rule**: never try to read the result of a widget-tree transition (a route pop, a `Future` that resolves via a rebuild) from inside a `tester.runAsync` block whose whole purpose is to let a REAL (non-fake-clock) I/O operation finish — the two need opposite pumping behavior. Do the real-I/O wait inside `runAsync`, alone; do all frame-dependent observation (`pumpAndSettle`, then reading state or awaiting a future that a rebuild resolves) strictly after it returns.

## A screen that owns its own date state silently resolved zone creates against the TIMELINE's day — four compounding bugs, found by inspecting a real user backup

**Symptom**, reported directly: `"this zone already exists for that day"` firing for days that visibly had no such zone; duplicate zones of the same type appearing on the grid; multiple identical cards in the Add Zone browser; and the count growing after each add/delete cycle.

**Diagnosis approach that actually worked**: rather than reasoning from the UI symptoms, a real exported backup (`amble-backup-…json`, 268 zone rows) was parsed directly and grouped by `recurrenceId` / `title` / `anchorDate`. That immediately reframed the problem: the user's two real zones ("Morning ritual", "heheh") existed as **seven** series, two of which shared 49 identical days with identical windows, and 156 of the 268 rows were redundant. No amount of UI-level reasoning would have surfaced that; one `python3 -c` over the backup did.

**Four distinct causes, compounding**:
1. **Every "Add Zone" with Repeat on mints a brand-new series.** `ZoneList.createZone` unconditionally does `recurrenceId: _uuid.v4()` — nothing checks whether that title already has a series. Adding the same zone twice produces two independent daily series, each generating its own ~8-week window.
2. **The generator is blind to other series.** `generateZoneRecurrenceInstances`' `takenDays` is built only from instances sharing the SAME `recurrenceId`, so series #2 cheerfully regenerates every day series #1 already occupies.
3. **A future-anchored template left orphans on delete.** `deleteZoneSeries` only detached a template that survived in the PAST; when the template itself was future-anchored it got deleted with everything else, stranding past rows with a live `recurrenceId` and no template. The generator can never regenerate such a series (it needs a template), but every stranded row still renders forever — 40 such rows in the backup.
4. **The collision check used the wrong day.** `_ZoneFormScreen` resolved `thisDay` from `selectedDateProvider` — which only the Timeline ever writes. The Weekly Zone Authoring Grid has its own `zoneGridWeekProvider` and never touches it, so every create started from the grid tested its collision (and anchored its new series) against whatever day the Timeline happened to be on.

**Fixes applied**: `showZoneFormScreen` gained an explicit `forDay` parameter (the grid passes `zoneGridWeekProvider`), used for BOTH the collision check and a new series' `anchorDateForRecurrence` so the two can never disagree; `deleteZoneSeries` now resolves survivors fresh from the repository after its sweep and detaches them whenever no template remains; the notice names the zone (`"Morning ritual" already exists on that day.`) via a new `ZoneFormResult.existingSameDayTitle`.

**Rule**: when a screen owns its own date/selection state, never let a shared dialog it opens fall back to a DIFFERENT screen's state provider — pass the value explicitly. A fallback that is correct for the original caller becomes an invisible, data-corrupting default the moment a second screen reuses the same dialog. And when a report involves "wrong/duplicate data," parse a real export before reading code: the shape of the data names the bug far faster than the UI symptom does.

**Follow-up — the two multiplying causes, and why a code fix alone was not enough.** The four causes above were fixed in two passes; causes (1) `createZone` always minting a fresh `recurrenceId` and (2) the generator's series-blind `takenDays` are the two that actually MULTIPLY rows, and they were deliberately held for confirmation before being fixed (they change what "two same-named zones" means app-wide). Once confirmed:

- `createZone` now REUSES an existing same-titled series instead of forking a parallel one — it adopts that series' `recurrenceId`, retunes its template in place, and re-materializes. This makes cause (2) unreachable in practice rather than patching the generator: with one series per title, there is no second series left to be blind to. Fixing `takenDays` to consider other series was considered and rejected — it would let two series co-exist while silently suppressing one's occurrences, which is a harder state to reason about than simply never creating the second series.
- **A code fix cannot repair data already on disk**, which is the lesson worth keeping. The user's install had 156 redundant rows that no amount of correct `createZone` behaviour would remove. `ZoneList.repairDuplicateZoneSeries()` runs at launch (before `materializeDueRecurrences`, so nothing re-fills a series about to be removed), collapses same-titled series to the template-bearing one, deletes the rest, and detaches orphans into plain standalone rows. It is idempotent and a no-op on healthy data.
- **Both the algorithm and its idempotency were dry-run against the real backup before shipping** — a Python transcription of the exact Dart logic, not of the intended behaviour: 268 → 113 rows, 155 deleted, 0 same-title-same-day duplicates remaining, and a second pass deleting nothing. Verifying a destructive repair against real data before it can touch a device is the practice to repeat; the earlier "intended algorithm" dry run would not have caught a mismatch between what was designed and what was written.

**Second follow-up — the duplication fix OVERCORRECTED, and the user caught it: "every attempt of adding zone ends up added in the list of templates."** The title-reuse branch above stopped duplicate series, but it did so by retuning the matched template in place — so re-adding "Morning ritual" at a different hour silently moved EVERY future occurrence, and (when Repeat was off) the reuse branch was skipped entirely and a brand-new standalone row landed in Settings' registry. Neither outcome was what "add this zone again at a different time" should mean.

**The missing concept was a third row shape**: `recurrenceId` set, `recurrenceRule` **null**, `anchorDate` set — a dated instance BELONGING to a series without being its template. That shape already worked everywhere; nothing had ever created one by hand. `ZoneListBody`'s registry filter (`!isRecurring || isRecurrenceTemplate`) excludes it, so it stays out of Settings; the grid's `_appliesOnDay` keys purely off `anchorDate`, so it still renders on its day. `createZone` now emits exactly that when the title matches an existing template, and never touches the template.

**Rule**: when a fix for "too many X" starts mutating shared state to avoid creating a row, check whether the thing being asked for is a DIFFERENT KIND of row rather than a variation of the one you have. Retuning shared state to represent a per-occurrence exception is a smell — the giveaway here was that the fix had to overwrite the template's own time, which is data no single occurrence should own. Also: two verification signals were available and neither was consulted before shipping the overcorrection — the registry filter's own predicate said plainly which shapes it lists, and the model already had a rule-less-instance shape in daily use from the generator.

## Two new zone-grid tests hung the suite in TEARDOWN, not in the test body — an unawaited Hive write outlived the test

**Symptom**: the two new live-badge tests in `zone_grid_screen_test.dart` hung for 10 minutes each and were reported as "did not complete" (no assertion failure). Across five runs the hang moved between the MOVE test and the RESIZE test, and once the whole file passed — which read convincingly as environment flakiness, especially with the machine's load average genuinely sitting at 19 on 16 cores at the time. Two runs were abandoned on that theory before it was tested.

**How it was actually found**: `debugPrint` markers placed after every single line of the failing test. All twelve printed, in order, including the one after the test's last assertion — so the test body ran to completion every time and the stall was always after it, in `tearDown`. That single observation invalidated the entire environment theory in one run.

**Cause**: ending a gesture calls `_onGestureEnd` → `_applyGroupMoveResult` → `notifier.commitZoneCascade(moves)`, which is `async` and does real `await repository.save(zone)` Hive I/O. The UI calls it fire-and-forget (correctly — no caller awaits it). Driving `onMoveEnd`/`onResizeBottomEnd` directly from a test outside `tester.runAsync` schedules that real write with nothing to let it run or finish; the test function returns immediately, and `tearDown`'s `await zoneBox.close()` then blocks forever on a box with a save still in flight. Both new tests had this; the older passing drag test in the same file did not, because it already wrapped its whole gesture in `runAsync`.

**Fix**: wrap only the gesture-ending callback in `tester.runAsync` with a short real delay (`await Future<void>.delayed(const Duration(milliseconds: 100))`), then pump outside it. 19/19 pass in 2 seconds.

**Rules**:
- A test that reports "did not complete" with no failed assertion is pointing at teardown or a dangling future, not at its own assertions. Instrument the body before theorising about the environment — a print after the last line settles it in one run.
- Whenever a test invokes a widget callback DIRECTLY (rather than via a simulated gesture), check what real async work that callback kicks off fire-and-forget. The widget tree's own "nobody awaits this" contract is fine in production and a teardown deadlock in a test.
- High machine load is a genuinely plausible cause of a nondeterministic hang and was true here — which is exactly why it was a trap. A real 2-second runtime after the fix showed load had never been the issue. Confirm an environment theory with a measurement that would distinguish it from a code bug, before acting on it.

## A bulk regex rewrite to add `widget.` prefixes silently corrupted named-argument LABELS

**What happened**: converting `_DayColumn` from `StatelessWidget` to `StatefulWidget` left ~48 `undefined_identifier` errors — every field reference in the moved `build`/`_blockFor` needed a `widget.` prefix. Rather than edit them by hand I ran a Python regex over the class's line range, prefixing a list of known field names.

**What it broke**: named arguments. `theme: widget.theme` became `widget.theme: widget.theme`, `onMoveStart:` became `widget.onMoveStart:`, and so on down the whole `ZoneGridBlock(...)` call. A label is not an expression, so this is not valid Dart — but the error count *dropped* from 52 to 12, which reads like progress. The remaining errors were also misleading: the analyzer reported the call as "missing required argument: theme, zone, top, height…" (because none of the labels matched any more) plus a parse error further down, none of which named the actual cause. A doc comment was corrupted too — "the day-column header's own" became "the widget.day-column header's own" — which no analyzer would ever flag.

**Fix**: repair by hand against text actually read back, not with a second pattern pass. Labels stay bare; only the values take the prefix.

**Rules**:
- A falling error count is not evidence a mechanical rewrite did the right thing. It can mean the errors moved somewhere less legible.
- Identifier-level regexes cannot see Dart syntax. In a named-argument call, `foo` on the left of `:` and `foo` on the right are different things, and a word-boundary pattern matches both. The same hazard applies to map keys, labels, and any place an identifier appears non-expressionally — plus comments and strings, which is how the prose damage happened.
- When a conversion produces dozens of mechanical errors, prefer the analyzer's own list as a worklist for real edits over a blind pass. Read the region back afterwards either way: the `expected_token` error here was visible only by looking at the file.

## A `GlobalObjectKey(zone.id)` silently collapsed seven legitimate copies of one widget into one

**Symptom**, reported directly: "when adding zone in settings it gets placed already on timeline but not edit zones .. screen." A zone created from Settings with Repeat OFF appeared on the Timeline but not on the Weekly Zone Authoring Grid.

**Two wrong hypotheses first, both cheap to rule out and worth recording as the wrong shape of question.** (1) That the grid's day filter differed from the Timeline's — it does not; `zone_grid_screen.dart`'s `_appliesOnDay` and `zone_containment.dart`'s `_zoneAppliesOnDay` are byte-identical, both returning true for a null `anchorDate`. (2) That a Settings-created *series* anchored to the Timeline's day (Settings passes no `forDay`, so `createZone` anchors to `selectedDateProvider`) and materialized instances outside the grid's visible week. That is a real latent bug and still unfixed — but it needs Repeat ON, and the report was Repeat OFF, so it was not this. Asking which, rather than assuming, is what closed off the wrong branch.

**Actual cause**: a non-recurring zone is dateless (`anchorDate == null`), which both surfaces read as "applies to EVERY day". The grid therefore builds seven blocks for it, one per column — each keyed `GlobalObjectKey(zone.id)`. A `GlobalKey` may be attached to only one widget in the tree at a time, so Flutter kept one and dropped the other six. The zone had not vanished; six of its seven blocks had been discarded, and the survivor was not necessarily in a column the user was looking at.

**How it was actually found**: a reproduction widget test asserting the zone renders in all 7 columns. It failed with `has length of <1>`, and the log carried Flutter's own `Multiple widgets used the same GlobalKey` assertion naming the two competing `Stack` parents. Reading the filter logic had already (correctly) exonerated the filters; only mounting the widget surfaced the key collision.

**Fix**: key on `(zoneId, day)` via a small `ZoneBlockKey` value class, so each column's block is its own identity. `_scrollZoneIntoView`, the key's only consumer, now walks the visible week and scrolls to the first day that has the zone mounted.

**Rules**:
- A `GlobalKey` built from a domain id is only safe while that id maps to exactly ONE widget. The moment a row can legitimately render in several places at once (every day, several columns, both panes of a split view), the key needs the position in it too — otherwise the framework silently keeps one and drops the rest.
- This class of bug is invisible to the analyzer and easy to misread as a data or filter problem, because the data is correct and the filter is correct. Mount the widget and count what rendered.
- `debugPrint`-free diagnosis: when a widget test fails on a count, read the whole log, not just the matcher output — the framework's own assertion above it often names the cause outright.

### Follow-up to the `GlobalObjectKey` entry above: the FIX broke every drag, and why

The entry above is correct about the collision but stops too early — it reads as though keying on `(zoneId, day)` were the end of it. It was not, and the rest is the part worth keeping.

**Keying on a freshly-constructed value object silently killed every drag-to-delete.** Five previously-green tests began failing on "hovering the drag over the target must arm it". Instrumenting `_onMoveUpdate` printed *nothing*: drag updates were never reaching the screen at all.

**Cause, from Flutter's own source** (`framework.dart`, `GlobalObjectKey`): *"Creates a global key that uses [identical] on [value] for its [operator==]"*. Its `==` is `identical(other.value, value)` and its `hashCode` is `identityHashCode(value)` — **reference identity, not `==`**. A `ZoneBlockKey` constructed inline in `build` is therefore a DIFFERENT key every frame no matter how correct its own `==`/`hashCode` are. A changed key destroys and recreates the element, and an element recreated mid-gesture drops the in-flight drag. `GlobalObjectKey(zone.id)` had worked only because `zone.id` is the same `String` instance from the same `Zone` object each build.

**Fix**: intern the key objects — `zoneBlockKeyFor(zoneId, day)` returns one canonical instance per pair from a map, so the same reference is reused across builds. The value class keeps `==`/`hashCode` purely for that map lookup; Flutter never consults them.

**Four wrong hypotheses preceded the right one**, all disproved by experiment rather than argument, and the sequence is the lesson: (1) the grid's day filter differed from the Timeline's — byte-identical; (2) the new empty-space `Listener` stole the gesture — removing it changed nothing; (3) the new `onHorizontalDrag*` handlers competed in the arena — removing them changed nothing; (4) the delete target's own key failed to resolve — the probe showed the handler was never called at all. Only a one-line bisect (revert the key, rerun) identified the culprit, and only reading the SDK explained it.

**Rules**:
- `GlobalObjectKey` compares by `identical`. Never build its value inline in `build()`. Use an object with stable identity across rebuilds, or intern it.
- A widget whose *key* changes is destroyed and rebuilt. If gestures, scroll position, or animation state die "for no reason", suspect key instability before gesture code.
- Two invalid baselines were attempted first — a `git stash` probe and a HEAD worktree — both meaningless because the entire feature directory is untracked, so there is no "before" to compare against. Check `git status` for untracked paths before reaching for either.

## `commitZoneCascade` re-validated overlap and threw — rejecting the cascade's own planned output

**Symptom**: zone pushing appeared not to work at all, even at the call sites where the cascade WAS correctly wired up.

**Cause**: `computeZoneCascadeMoves` plans a set of moves whose whole purpose is to resolve an overlap by relocating neighbours. `commitZoneCascade` — the function that writes those moves — then re-ran `weeklyWindowConflicts` over the planned result and threw `StateError` if it found any overlap. Since a cascade's intermediate plan legitimately describes zones at new windows, the commit path could reject the very output the algorithm had just computed. A test had even pinned this as correct behaviour (`'group resize cannot make selected placements overlap each other'`, asserting `throwsStateError`), so it read as intentional.

**Fix**: `commitZoneCascade` now validates only genuinely impossible windows (off the end of the day, or inverted). Resolving overlap is the caller's job, via `resolveZonePlacement`, and the commit path trusts it.
