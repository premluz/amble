...
## [2026-09-09] Android App Actions — five native entry points, emulator fulfillment verified

Implemented Add Task, Add Note, Add Zone, Read Day Summary and Remove Task through Android App Actions. Commands open Amble and use the shared Dart parser/repository handlers already built for iOS. MainActivity shares one Flutter engine across launches/recreation, avoiding a second Hive cache. Added custom capability/query XML, native result/input dialogs and optional offline TTS feedback. No new dependencies or permissions; no parser, repository, Quick Capture or Slack changes.

Android resolution is explicitly different from Siri: an Amble-owned native picker shows matching scheduled tasks and requires a tap before deletion, including a single match. Multiple fuzzy matches never silently choose the first. AppFunctions was researched but not implemented: Android 16+ plus restricted Gemini preview access does not match the connected Android 15 phone. App Actions custom intents require English (US) and Google's preview/Play setup for voice discovery. See [ANDROID_APP_ACTIONS.md](ANDROID_APP_ACTIONS.md) for exact commands and remaining steps.

**Verified:** debug APK builds and installs on the connected phone and existing API 34 emulator. The emulator renders the normal app and fulfills all five actions via direct Android intents. Tested two task captures, note text preservation, valid zone creation, overlapping/reversed-time rejection, real summary data, ambiguous removal, cancellation preserving both tasks, exactly-one deletion and persistence after a cold restart. Existing exact-alarm permission settings appeared during the first save/startup refresh; returning to Amble let the flow continue. Clearly named Assistant probe fixtures remain on the emulator.

**Checks/limits:** 26 intent tests pass; analyze retains only two pre-existing `modalTitle` warnings. Full suite reached 905 passes / 16 failures before the documented List-mode hang; all 16 failures match the previously reproduced baseline, and the run was stopped at 75 seconds. The physical phone installed successfully but remained locked for UI verification. **Actual Google Assistant voice phrases tested: none; audible TTS unverified.** Follow-up: unlocked-phone UI pass and Google Assistant preview/real phrase tests. AppFunctions/Gemini is a separate future implementation choice, not delivered by this App Actions pass.

## [2026-09-09] List view's "Show duration" and new "Show time (from-to)" split into independent toggles

Reported directly from an annotated screenshot: the dev "Show duration" setting was baking the `(1h)` suffix onto the SAME string as the time range, so List view's time and duration couldn't be toggled independently, and a new "Show time (from-to)" setting was requested to control the time range on its own — both scoped to List view only, explicitly never Task (spatial) view ("on this view we'd never show this"). Confirmed via a direct question that the two should be independent toggles (either, both, or neither on), not mutually-exclusive modes.

Added `DevTimelineTaskTimeRangeVisible` (`core/dev_config.dart`, default true — matches List view's existing always-on time range) alongside the existing `DevTimelineTaskDurationVisible` (default false), and a matching Settings row. Rewrote the time-label string building in every List-view render path to treat the two as independent optional pieces joined with `[?a, ?b].join(' ')`, gated behind the existing `alwaysShowTime`/`compactText` List-mode signal so Task view's split-layout column (where `durationVisible` alone still hides the WHOLE time+duration display, unchanged) and Zone view (which never sets `compactText` at all and needed no code change) are both structurally untouched: `TaskCapsuleTextRow` and `OverlapClusterBlock`'s `_ClusterTaskRow`/`_ClusterEventRow` (fixed earlier), then `TaskCapsuleBlock` itself and its `_buildSplit`/`_DraggableTaskBlock` chain in `timeline_screen.dart` (completed this session) — the widget actually rendering List view's ordinary, non-clustered task rows, which had the identical conflation bug independently. `ExternalEventCapsuleBlock` was deliberately left untouched — it only ever toggles a plain time range with no duration suffix to split out, so there's nothing for the new setting to affect there.

**Checks:** `flutter analyze` clean (2 pre-existing `modalTitle` warnings only). `task_capsule_text_row_test.dart` (18/18), `overlap_cluster_block_test.dart` (22/22 after fixing a parens-format regression the rewrite introduced — `(1h)` had silently become bare `1h` until a test caught it), `task_overlap_layout_test.dart`/`collapsed_stack_layout_test.dart` (34/34) all pass. `test/features/timeline/` as a whole still exhibits the documented hang and was not run to completion, per existing convention.

## [2026-09-09] Two more List-view-only dev filters: "Only Amble tasks" and "Only important"

Requested directly, same session as the duration/time-range split above: "config for Only amble tasks (affects list view only) and config (only important)". Added `DevTimelineListOnlyAmbleTasks` (hides every imported `ExternalCalendarEvent`) and `DevTimelineListOnlyImportant` (hides every `Task` with `isImportant == false`), both defaulting off and independent of each other, plus matching Settings rows.

Unlike the duration/time-range pair (which only reformats each row's own text), these two remove rows entirely, so they had to be applied at the single choke point both Task and List mode already share: `_DayTimelineState.build()`'s `blocks` list, which every downstream layout call (`layoutOverlappingTasks`, `detectOverlapClusters`) consumes. Filtered `tasks`/`widget.externalEvents` into `filteredTasks`/`filteredExternalEvents` there, gated behind `!widget.showHourLabels` (the same List-mode signal used throughout this file) — Task view's own `blocks` construction is untouched regardless of either toggle's value. The day's scrollable range (`_visibleRange`) deliberately keeps reading the UNFILTERED task list, so hiding tasks in List mode never shrinks or jumps the scroll bounds.

**Checks:** `flutter analyze`/`dart format .` clean. New `list_mode_task_filters_test.dart` (6/6) drives the real `TimelineScreen` end-to-end (mirroring `task_event_lane_sharing_test.dart`'s harness) and confirms both toggles filter correctly in List mode, leave Task view untouched, and default to today's unfiltered behavior when off — the two "hides" cases were confirmed to fail against the pre-fix code before being kept. `task_event_lane_sharing_test.dart`/`list_mode_clustering_test.dart` rerun clean, no regressions.

## [2026-09-09] Fixed a real crash from the two new List-view filters: zone bands force-unwrapping a null

Reported directly: "Error check operator used on a null value (also no time no duration would apply to imported tasks)". Traced to `collapsedZoneBands`'s `_rawMemberBand` (`task_overlap_layout.dart`): a zone's containment is resolved from the FULL task/event lists regardless of either new List-view filter, so a zone whose only member got hidden by "Only important" or "Only Amble tasks" still looked non-empty to the empty-check guarding the placeholder-band branch, fell into the real-member branch, found nothing in `taskTops`/`externalEventTops` to consider, and force-unwrapped two `null`s (`minTop!`, `maxBottom!`) — exactly the reported crash.

Fixed both ends: `_rawMemberBand` now returns `null` (not a forced non-null record) when nothing was actually considered, its caller falls back to the same placeholder-band treatment a genuinely empty zone gets, and `_collapsedZoneBands` in `timeline_screen.dart` now resolves containment from the FILTERED `filteredTasks`/`filteredExternalEvents` (matching what `blockTops` was actually built from) rather than the raw `widget.externalEvents`, so a filtered-out member isn't even counted as "in the zone" in the first place. Two new regression tests in `list_mode_task_filters_test.dart` seed a zone whose sole member is hidden by each filter and assert `tester.takeException()` is null — both confirmed to reproduce the exact "Null check operator used on a null value" crash against the pre-fix code before the fix was applied.

Separately, per the parenthetical and a direct follow-up confirmation: imported calendar events in List view now NEVER show a time range or duration label, regardless of either dev toggle (`_ClusterEventRow` in `overlap_cluster_block.dart`, `_ExternalEventTextRow` in `external_event_capsule_block.dart`) — an event's "duration" was only ever a computed span (`end - start`), never something the user set, so the duration toggle never had a real concept to show for events in the first place. Task view's own event time display is unchanged. Updated `external_event_capsule_block_test.dart`'s List-mode test to match (was asserting the old "always show time in List mode" behavior).

**Checks:** `flutter analyze`/`dart format .` clean. `collapsed_zone_bands_test.dart` (12/12, no regression on the pure unit tests), `overlap_cluster_block_test.dart` (11/11), `external_event_capsule_block_test.dart` (10/10, includes the two rewritten/new List-mode cases), `task_event_lane_sharing_test.dart` (13/13), `list_mode_task_filters_test.dart` (8/8, includes the two new crash-repro cases) all pass.

## [2026-09-09] Zone view rewritten from spatial timeline to a plain non-spatial list

Requested directly: "current zone view make non spatial.. just list of zones one by one with 4px gap." Follow-up confirmations settled the two open questions before building: zones keep their existing tasks-inside-container look (`ZoneContainerBlock`, unchanged), and unzoned tasks/unmatched external events are kept, interleaved chronologically between zones rather than dropped or moved to a separate section; drag-to-move/resize a zone and drag-and-drop task reassignment are gone entirely (no time axis to drag along), with a zone's own fields now edited by tapping its header to open `showZoneFormScreen` — the same precedent Settings' plain `ZoneListBody` already used.

`ZoneDayTimeline` shrank from a 1130-line `StatefulWidget` (time-axis math, `Positioned`/`Stack` geometry, live drag/resize/move state, scroll-position-restore plumbing) to a small `StatelessWidget`: `resolveZoneContainment`'s output (zone containments + unzoned tasks) plus the caller's own external-events list get merged into ONE chronologically-sorted sequence and handed to a single `ListView.separated` (`theme.spacingXs`, 4px, matching the request exactly). Each row renders as `ZoneContainerBlock` (zones — completely unchanged internally, since it was already a self-contained `Column`, never requiring its own `Stack` ancestor), `TaskCapsuleBlock` with `compactText: true`/`durationIndicatedBySize: false` (unzoned tasks, matching List view's own flat-row look), or a small new `_UnzonedEventRow` (unmatched external events — `ExternalEventCapsuleBlock` couldn't be reused here since it builds its own outer `Positioned` and requires a `Stack` ancestor). `TimelineScreen`'s call site dropped every now-gone parameter (`pixelsPerMinute`, `onReassign`, `readViewedMinutes`/`onViewedMinutesChanged`, `editModeEnabled`, `onZoneResize`/`onZoneMove`, `multiTaskEditMode`, `selectedZoneId`) and wired `onZoneHeaderTap` straight to `showZoneFormScreen`.

One real cross-cutting fix: `ZoneContainerBlock.onHeaderTap` previously only ever fired while `editModeEnabled` was true (it existed solely for Task view's multi-task zone-select route). The new Zone view's whole interaction model is "tap the header to edit," with `editModeEnabled` always false — so the gating was widened to `onMoveStart == null || editModeEnabled`, meaning a bare `onHeaderTap` with no move contract wired (the new Zone view's exact shape) fires unconditionally, while Task view's route (which wires `onHeaderTap` alongside a real move contract) keeps its original Edit-Mode gate untouched. Confirmed via a test that fails without the fix and via the full `zone_multi_task_selection_test.dart`/`zone_task_view_move_resize_test.dart` suites rerun clean.

Two now-fully-obsolete test files (`zone_day_timeline_position_test.dart` — pixel-parity with the Task view's `ZoneBackgroundBlock` on a time axis that no longer exists; `zone_move_end_to_end_test.dart` — driving `onMoveStart`/`onResizeBottomStart` through the real `TimelineScreen`) and `zone_day_timeline_move_resize_test.dart` (drove the same removed callbacks directly) were deleted rather than adapted — the entire feature they tested is gone by design, not regressed. New `zone_day_timeline_list_test.dart` (9 tests) covers the replacement: one `ZoneContainerBlock` per zone, exact 4px separation, zones' own tasks/matched events render inside their container, unzoned tasks/unmatched events render as their own flat rows, chronological interleaving (not zones-first), header tap fires `onZoneHeaderTap`, and no move/resize callback is ever wired. Two new cases in `zone_container_block_test.dart` cover the `onHeaderTap` gating change specifically, one confirmed to fail against the pre-fix gate.

Flagged, not touched (out of scope for "make Zone view non-spatial"): `TaskList.rescheduleTaskWithZone` and `DevZoneViewPixelsPerMinuteProvider` (plus its Settings row) are now genuinely dead code — both existed solely to serve Zone view's removed drag-and-drop/spatial-scale features. Left in place since removing them wasn't requested; a real follow-up cleanup if wanted.

**Checks:** `flutter analyze`/`dart format .` clean. `zone_day_timeline_list_test.dart` (9/9, new), `zone_container_block_test.dart` (9/9, includes 2 new), `zone_multi_task_selection_test.dart` (18/18), `zone_task_view_move_resize_test.dart` (4/4), `zone_background_block_test.dart` all pass — 41 total across the affected files, no regressions in Task view's own zone editing.

## [2026-09-10] "Show time (from-to)" now also applies to the non-spatial Zone view

Requested directly: "Hide/show start end should also affect zone view." Gap traced to yesterday's Zone-view rewrite: `DevTimelineTaskTimeRangeVisible` (the List view "Show time (from-to)" Settings toggle) was never threaded into `ZoneDayTimeline`/`ZoneContainerBlock` at all, so every row there always showed its start-end range regardless of the Settings toggle.

Threaded `timeRangeVisible` through the same chain `durationVisible` already used: `ZoneDayTimeline.devTimeRangeVisible` (wired from `TimelineScreen`'s existing `devTimelineTaskTimeRangeVisibleProvider` watch) down into `ZoneContainerBlock.timeRangeVisible`, which reaches its own `_Header` (the zone's own start-end range — its title's separate `(duration)` suffix is unaffected, matching how a zone's own duration and its time range are already two different pieces of text), `_ZoneTaskRow`, and `_ZoneExternalEventRow`; separately reaches the unzoned-task `TaskCapsuleBlock` rows and the new `_UnzonedEventRow` in `ZoneDayTimeline` itself. Every row's time-label computation was rewritten from a single ternary (duration suffix on/off) to the same `[?timeRange, ?durationLabel].join(' ')` independent-pieces pattern `TaskCapsuleTextRow` established for List view — `timeRangeVisible` and `durationVisible` are two separate toggles here too (either, both, or neither can be on), not one gating the other.

One deliberate scope note: List view's imported-event rows never show time/duration at all regardless of either toggle (a prior, separately-confirmed decision, since an event's "duration" is only ever a computed span). That decision was scoped to List view specifically and was never extended to Zone view's own event rows — `_ZoneExternalEventRow`/`_UnzonedEventRow` keep respecting both toggles normally, unaffected by that List-view-only carve-out.

**Checks:** `flutter analyze`/`dart format .` clean project-wide (only pre-existing, unrelated warnings from untracked `review_probe`/`review_zone_probe` scratch files remain). 15 new/updated cases across `zone_day_timeline_list_test.dart` and `zone_container_block_test.dart` cover the header, in-zone task row, unzoned task row, unzoned event row, and the two-toggles-independent case — several confirmed to fail against the pre-fix code before being kept. Full Zone-view suite (`zone_container_block_test.dart`, `zone_day_timeline_list_test.dart`, `zone_multi_task_selection_test.dart`, `zone_task_view_move_resize_test.dart`, `zone_background_block_test.dart`) rerun clean, 51/51, no regressions in Task view's own zone editing.


## 2026-09-10 — Current working-tree code and architecture review

Executed the requested review of zone materialization, timeline layout, sheets, press feedback and padding. Findings and recommendations are recorded in `docs/CODE_ARCHITECTURE_REVIEW.md`: ten ranked correctness findings, architectural risks, test-quality gaps and standing-constraint compliance. Nine temporary regression cases reproduced seven finding areas; temporary tests were removed after evidence collection. No production fixes or dependencies were introduced, and concurrent user changes were preserved.

Validation: `flutter analyze --no-pub` completed with two unused `modalTitle` warnings. The full test run reached 933 passes and 10 failures before stalling in the existing List clustering widget test; terminated at 75 seconds, so the suite did not complete. No real-device animation or Siri validation was performed. Recommendations are review proposals, not adopted architecture decisions.

## [2026-09-10] New-task entrance: spring/bounce scale-up, not a plain fade

Requested directly: "Animation on new task added should ba scale up (larger than the task pill e.g. 120% and bounce to 100% widht, and kind of spring easing." The existing entrance (`_DraggableTaskBlockState`'s `_entranceProgress`, driving `TaskCapsuleBlock`'s 5-part opacity stagger) never touched scale at all — a newly created task's pill simply faded in at its resting size.

Added `entranceScaleFor` (`timeline_screen.dart`, top-level and pure) — a custom "back ease" cubic (the same family `Curves.easeOutBack` is built from, `f(t) = 1 + (c1+1)(t-1)³ + c1(t-1)²`) tuned to a `c1` constant whose overshoot peaks at exactly 1.2 (120%), found by binary search since `Curves.easeOutBack`'s own fixed constant (`1.70158`) only reaches ~1.087 and can't be linearly rescaled to hit an arbitrary peak (confirmed by direct sampling before reaching for a custom curve). `f(0) = 0` (pops in from nothing, not from resting size) and `f(1) = 1` (settles exactly at rest) fall out of the same cubic family's endpoints regardless of `c1`. Applied inside `_DraggableTaskBlock`'s existing entrance `TweenAnimationBuilder`, split so the RAW (linear) progress value feeds both the pre-existing `easeOut`-curved opacity stagger and this new, differently-curved scale — the two effects needed separate curves applied to one shared timeline rather than one already-curved number doing double duty. Drag-lift (`_liftScale`, unrelated pre-existing behavior) still takes priority whenever a block is actively being dragged.

**Checks:** `flutter analyze`/`dart format .` clean. New `entrance_scale_test.dart` (5/5) locks in the exact curve shape — starts at 0, peaks at exactly 1.2, settles to exactly 1.0, genuinely overshoots past 1.0 (not just a fade), stays within [0, 1.2] throughout — as pure-math coverage, since driving the real animation through `_DraggableTaskBlock`'s `fadeInOnFirstBuild` needs the full `TimelineScreen` + `recentlySavedTaskProvider` chain for a deterministic curve computation that doesn't need it. Caught and fixed two of its own math errors before landing: an initial hand-derived "peak = 1.10" claim for `Curves.easeOutBack` that was actually ~1.087 when sampled directly, and a first linear-rescale formula that produced a nonsensical negative starting scale. `tap_empty_space_quick_create_test.dart`/`multi_task_selection_test.dart`/`task_capsule_text_row_test.dart` (43 total) rerun clean, no regressions in task-creation or capsule rendering.

## [2026-09-10] Zones can now be deleted via drag-to-delete-target, matching tasks

Reported directly: "What about zones cant see remove? when in edt move dragging zone should remove zone appear like with tasks." Investigation found `ZoneList.deleteZone` already existed on the provider but had ZERO call sites anywhere in the UI — there was genuinely no way to delete a zone from the app at all before this.

Wired zones into the exact same shared delete-target mechanism `_DraggableTaskBlock` already uses in the Spatial Task View, rather than building a second target: `_DraggableZoneBlock` gained `deleteTargetKey`/`onDeleteTargetVisibilityChanged`/`onDeleteTargetArmedChanged`/`onDeleteZone`, all threaded from `_DayTimelineState`'s existing `_deleteTargetKey`/`_deleteTargetVisibleCount`/`_deleteTargetArmed` fields (the same ones tasks read from). `_commitMove` (fired on a zone move-drag's release) now checks a delete-drop FIRST, before any move commit — mirroring `_DraggableTaskBlockState.onDragEnd`'s exact ordering and short-circuit shape — and calls the new `onDeleteZone` callback, wired at `TimelineScreen`'s own level to `zoneListProvider.notifier.deleteZone(zone.id)`. No recurring-scope disambiguation (unlike `removeTask`'s this-instance/all-instances prompt) — `deleteZone` has no series concept to ask about, so a materialized recurring instance deletes the same way a plain zone does, matching that provider method's own existing, deliberate per-instance design.

Scoped to the Spatial Task View only (`_DraggableZoneBlock`/`ZoneBackgroundBlock`) — the non-spatial Zone view (`ZoneDayTimeline`, rewritten 2026-09-09) has no drag/move at all any more by design, so "remove like tasks" has no equivalent gesture to hook into there; a zone is still deletable from that view only via its header tap → `showZoneFormScreen`, unchanged.

**Checks:** `flutter analyze`/`dart format .` clean. Two new tests in `zone_task_view_move_resize_test.dart`: dropping a zone move-drag on the delete target deletes it (confirmed to fail against the pre-fix code before being kept) and an ordinary move that never touches the target leaves the zone untouched. Full `zone_task_view_move_resize_test.dart`/`zone_multi_task_selection_test.dart`/`zone_background_block_test.dart`/`zone_container_block_test.dart`/`zone_day_timeline_list_test.dart` suite (53 total) and the task-side `edit_mode_delete_target_test.dart`/`multi_task_group_delete_test.dart` (6 total) rerun clean — no regressions in the shared delete-target mechanism or existing zone move/resize/multi-select behavior.

## [2026-09-10] Remove icon button added to Zone, Template, and Category edit screens

Requested directly: "Edit zone screen should have remove icon button same as w[ith] edit t[a]sk. Edit template also remove icon b[u]tton add. And Edit category also." `StepScaffold` (the shared chrome all four edit screens are built on) already had a generic `onSecondaryAction`/`secondaryActionIcon` pair, explicitly documented as "shared across features (task edit, Zone add/edit)" — Zone and Template's own edit screens simply never wired it up, so this was mostly threading `_delete` methods through an existing, already-shared widget rather than building new UI.

**Zone** (`zone_form_screen.dart`) and **Template** (`task_template_form.dart`) were straightforward: both already had a working, unguarded `deleteZone`/`deleteTemplate` provider method with zero UI call sites (Zone's own gap was fixed in yesterday's drag-to-delete-target session; Template's was simply never wired to this screen specifically, though its own action sheet already used it). Both new `_delete` methods mirror `task_detail_sheet.dart`'s own `_delete` exactly: edit-only, no confirmation dialog, pop-then-delete ordering so `ref` is never read after the screen unmounts.

**Category** was a real scope conflict, flagged before building: `CategoryRepository` had NO delete method at all, per a recorded v1 decision (docs/DECISIONS.md) that deliberately left "deleting a category referenced by existing tasks" unresolved. Building the button meant designing that behavior first. AskUserQuestion settled it: REASSIGNMENT, not blocking or orphaning — `CategoryList.deleteCategory` (new) reassigns every referencing `Task.categoryId` to `BuiltInCategoryIds.general` before deleting the row, chosen over the `templateId`/`zoneId`-style "leave it stale" precedent specifically because `categoryId` drives real, currently-visible capsule color/emoji, unlike those two inert reference fields. `CategoryRepository.deleteCategory` added to both the interface and `HiveCategoryRepository`. A built-in category can never be deleted — enforced both in the notifier (silent no-op guard) and in the UI (`add_category_modal.dart` only offers the button for `!isBuiltIn`), since `general` itself is the fallback the reassignment logic depends on existing. See docs/DECISIONS.md's own updated entry for the full reasoning.

**Checks:** `flutter analyze`/`dart format .` clean project-wide. New tests: `zone_form_screen_test.dart` (+2), `task_template_form_test.dart` (+2), `add_category_modal_test.dart` (+3, including the built-in-can't-delete case and the reassignment-actually-happens case), `hive_category_repository_test.dart` (+2) — 9 new cases total, each confirmed to fail against the pre-fix code before being kept. Full suite across all four affected test files (37 tests) passes with no regressions. One test-infra fix along the way: `zone_form_screen_test.dart` needed a `FakeNotificationService` override it didn't have before — `ZoneList.deleteZone` awaits `cancelForZone` directly (unlike `_save`'s own fire-and-forget notification sync), so the delete test hit a real `MissingPluginException` without it; flagged as a pre-existing inconsistency in `deleteZone`, not fixed (out of scope for this session).

## [2026-09-10] Task Size setting: badge and font decoupled to different rungs

Requested directly: "Current medium size but with font size from small should be small... Current large but with the font size medium should be medium... large should be larger task pill but font size same as medium." Before this, "Task size" (sm/md/lg) moved a badge rung and a same-named font rung together in lockstep (sm=20px/12pt, md=24px/14pt, lg=28px/16pt); the request decouples them so each named option pairs a badge with a font rung ONE STEP SMALLER than before.

Confirmed the exact new badge size for "Large" via AskUserQuestion (the request said "larger" without a number): 32px, the next existing `SpacingPrimitives` rung above 28px, continuing the same even-step pattern (20→24→28 becomes 24→28→32) rather than inventing a new interval. Added `AmbleTheme.sizeTaskBadgeXl` (32px, both palettes) — a genuinely new rung, since "Large" now needs a badge bigger than the old `sizeTaskBadgeLg` ever provided. All the mapping logic lives in one place, `main.dart`'s `_resolveTaskSize` (already the single documented spot this happens, per its own doc comment) — new pairing: sm → badge md (24) + font sm (12); md → badge lg (28) + font md (14); lg → badge xl (32, new) + font md (14, same as md's own font — the exact "large should be larger pill but font size same as medium" requirement). `textTaskTitleLg` (16pt) is no longer referenced by the mapping at all; left in place as a named token rather than removed, in case a future rung needs it.

No call site outside `semantic_theme.dart`/`main.dart` reads the raw `sizeTaskBadgeSm/Md/Lg`/`textTaskTitleSm/Md/Lg` fields directly (confirmed by grep) — every real consumer (`TaskCapsuleBlock`, `ZoneContainerBlock`, List view's collapsed scale) reads only the resolved `sizeTaskBadge`/`textTaskTitle` pair, so this was a pure remap with zero call-site changes needed.

**Checks:** `flutter analyze`/`dart format .` clean. `task_size_scale_test.dart` rewritten to test the ACTUAL new `_resolveTaskSize` pairing (its old version demonstrated the now-incorrect same-name pairing, which would have shipped as misleading documentation-via-test) — 11 cases, including one asserting the md/lg options now share the exact same font size while lg's badge stays strictly larger. `hive_preferences_repository_test.dart`/`shadow_lift_token_test.dart` (the other tests touching `AmbleTheme.copyWith`/`lerp`) rerun clean, confirming the new required constructor field didn't break either. No test anywhere constructs `AmbleTheme(...)` directly (all use `.light`/`.dark`/`.copyWith`), so the new field needed no test-fixture updates elsewhere.

## [2026-09-10] Accent "time on the right" mechanism reused for the pending draft pill and Edit Mode's wiggling task

Requested directly: "We have that [mechanism] to show on the right time in accent color bg, we use it for quick task add on long press[;] we need to use it... when quick new add task is dropped and wiggly showing start and end of task... until it's scheduled or closed[;] also in edit mode for selected (wiggly tasks) we need to show it." The mechanism in question is `place_task_line.dart`'s `_PlacementLine` — a hairline plus an accent-colored time pill on the right edge, previously shown only while a long-press-drag is in progress on empty Timeline space.

Promoted that visual into a new, standalone `TaskEdgeTimeLabel` widget (`task_edge_time_label.dart`) — the same hairline+pill render, now reusable wherever a task's own start/end needs it, rather than staying private to the placement-line file. `_PlacementLine` itself now just wraps `TaskEdgeTimeLabel` in its own `FractionalTranslation` positioning; its rendered output is unchanged (confirmed via the existing `place_task_line_test.dart` suite).

Wired into two new call sites, both confirmed via AskUserQuestion to use TWO separate edge labels (one per edge, not a combined "start - end" string) and to stay visible for the WHOLE time the block is wiggling, not just during an active drag: (1) `PendingTaskPill` (the tap-to-create draft) now shows its own live start/end, recomputed each frame from the same `_dragOffset`/`_resizeOffset`/`_resizeTopOffset` running totals `_DraggablePendingTaskPillState._commitMove`/`_commitResize`/`_commitResizeTop` already track — read-only for the label, never written to the draft provider until an actual commit. (2) `_DraggableTaskBlock` (a real task) now shows its own start/end whenever `wiggleEnabled` is true — the same flag that already drives its `EditModeWiggle`, covering both single-task long-press-armed editing and multi-task-mode selection — suppressed during an active drag/resize exactly like the wiggle itself already is, since a live drag already shows its own preview via the pill's own position.

**Checks:** `flutter analyze`/`dart format .` clean. New `task_edge_time_label_test.dart` (5/5) covers the promoted widget directly. `pending_task_pill_test.dart` gained 3 cases for the new labels (existing 8 cases updated for the new required `startTime`/`endTime` params, all still passing). New `task_edit_time_labels_test.dart` (3/3) drives the real `TimelineScreen` through long-press-arm and tap-to-close, confirmed to fail against the pre-fix code before being kept — caught and fixed a real test-harness mistake along the way (closing via an empty-space tap collided with this same Timeline's own tap-to-quick-create feature, which fired its own NEW pending draft with its own labels, confounding the count; fixed by closing via a tap on the task itself instead, matching `armed_edit_task_test.dart`'s own established close path). `place_task_line_test.dart` rerun: 7/8 pass; the 1 failure (`a long press still shows the line and fires onPlaced, unaffected by onTapAt being wired up`) was confirmed via `git stash` to fail identically on unmodified `main` — a genuine pre-existing issue, not caused by this session's refactor, left unfixed as out of scope.

**Follow-up same session**: reported directly, "shouldn't have lines just the hour on bg" — the hairline was wrong for the two NEW uses specifically. Confirmed via AskUserQuestion: keep the hairline on the ORIGINAL long-press placement line (it marks a drop point on otherwise-empty background, where a line usefully draws the eye), but drop it from the pending draft pill and Edit Mode's wiggling task (both sit ON an already-visible block, where a second full-width line reads as noise). Added `TaskEdgeTimeLabel.showLine` (default `true`, preserving the placement line's own look with zero change there), set `showLine: false` at the two new call sites. `task_edge_time_label_test.dart` gained 3 cases for the flag (7/7 total now); `pending_task_pill_test.dart`/`task_edit_time_labels_test.dart` rerun clean, unaffected (their own assertions check the time text, not hairline presence).

## [2026-09-10] Live time indicator on the LEFT (hour gutter) during a move-drag

Requested directly: "Display time on left hand side on top of the day times. When dragging (moving) can't see hour changeing. only when resizing can see." Traced the existing `dragPreviewStartsAt` plumbing (`_DraggableTaskBlockState.build()` → `TaskCapsuleBlock`/`TaskCapsuleTextRow`) end to end and found it structurally correct — the live preview time IS already threaded through during a move — so this wasn't a broken-code bug, just not prominent enough to notice while dragging. Confirmed via AskUserQuestion: add a new, more visible indicator reusing the same accent-badge style as `TaskEdgeTimeLabel`, move only (not resize — the pill's own inline text already covers resize, confirmed directly as sufficient).

Implementation lives entirely in `_DraggableTaskBlockState._buildSplit` (`timeline_screen.dart`) — the one render path that actually ships (`splitLayout: true` is hardcoded at its production call site; the parallel combined-layout branch is dead code, left untouched). Added a `Positioned(top: 0, left: -widget.left, ...)` wrapping a `TaskEdgeTimeLabel(showLine: false)`, gated on `_isDragging && !widget.compactText` (Task view only — List mode has no hour gutter for this to sit over). `widget.left` always equals the hour gutter's own width in this render path, so `-widget.left` walks the badge back from the block's local origin to the day column's true x=0, landing it exactly over the gutter. Deliberately left the `Positioned` unconstrained on width (no `width: widget.left`) after an initial attempt overflowed — the badge's own pill content (e.g. "12:45 PM" plus padding) is wider than the ~56-80px gutter itself, so it now sizes to its own intrinsic content and spills slightly past the gutter rather than being squeezed and clipped.

**Checks:** `flutter analyze`/`dart format .` clean project-wide (only the 2 pre-existing unrelated `modalTitle` warnings remain). New `drag_move_time_label_test.dart` (4/4): no label when idle, label appears with the live snapped time during a move-drag, disappears on release, and carries `showLine: false`. Confirmed 3 of the 4 fail against the pre-fix code before keeping the fix (the 4th, "no label when idle," doesn't depend on it). Hit and fixed a real layout bug along the way: the first attempt constrained the badge to exactly `widget.left`'s width, which overflowed for any task whose formatted time didn't fit that width — fixed by dropping the width constraint entirely. Reran `zone_task_view_move_resize_test.dart`, `zone_container_block_test.dart`, `zone_day_timeline_list_test.dart`, `list_mode_task_filters_test.dart`, `entrance_scale_test.dart`, `task_edit_time_labels_test.dart`, `task_edge_time_label_test.dart`, `pending_task_pill_test.dart` — all pass, no regressions.

## [2026-09-10] Settings restructured into grouped buttons, each a pushed sub-page

Requested directly: Settings should follow the existing "Manage zones"/"Manage categories" pattern (a button that pushes a full page with its own back button + title) everywhere, not just for those three — organized into five bracket groups: `[Permissions, Appearance]`, `[Zones, Templates, Categories]`, `[Calendars, Slack]`, `[Backup]`, `[About, Developer]`. The single ~1940-line `settings_screen.dart` (one long scrolling page of inline panels) is now a short top-level screen listing plain label+chevron rows (confirmed via AskUserQuestion — no summary text under each label, unlike the pre-existing `_ManageLinkRow`), grouped into the five cards above, each row pushing its own screen.

New shared scaffolding: `settings_detail_scaffold.dart` (`SettingsDetailScaffold` — the back-button+title chrome, mirroring `ZoneListScreen`'s own header exactly; `SettingsLinkRow` — the plain top-level row) and `settings_panel.dart` (`SettingsPanel`, the rounded-card background, extracted from the old private `_SettingsPanel` so every new page can share it without depending on `settings_screen.dart`). New pages, one file each, carrying their own previously-inline state/logic verbatim: `permissions_settings_screen.dart` (notification status + system settings link), `appearance_settings_screen.dart` (theme mode + task size), `calendars_settings_screen.dart` (display calendars + sync-to-calendar, previously `_CalendarSettingsPanel`), `slack_settings_screen.dart` (webhook fields + test send), `backup_settings_screen.dart` (export/import, keeping the `debugAutoTriggerExport`/`debugAutoTriggerImport` scaffolding hooks), `about_settings_screen.dart` (app version). Zones/Templates/Categories keep pushing their existing `showZoneListScreen`/`showTemplateListScreen`/`showCategoryListScreen` directly, confirmed via AskUserQuestion — no new intermediate "Manage" page, since they already worked exactly this way.

`developer_settings_screen.dart` absorbed everything the request named as moving out of always-visible top-level sections: the "Scheduling" section (`Prevent overlapping tasks`, previously NOT debug-only — moved into Developer under its own sub-heading per direct confirmation, so this toggle is now unreachable in release builds), the "Timeline" section (hour labels, connectors, completion checkbox, overlap clustering), and the "Zone view" display toggle (previously living inside the old "Manage" panel, now living in Developer alongside its own dev-only mirror `devZoneViewInCycleProvider`, which was already there). The feature-flagged "Tracked behaviors" section, not mentioned in the original bracket list, was folded into Developer too per AskUserQuestion rather than kept as its own top-level button.

**Checks:** `flutter analyze`/`dart format .` clean project-wide (same 2 pre-existing unrelated warnings only). Fixed a real provider-import mistake caught by analyze: `calendarSyncTargetIdSettingProvider`/`calendarDisplayIdsSettingProvider` actually live in `preferences_providers.dart`, not `calendar_providers.dart` — `calendars_settings_screen.dart` initially imported the wrong file. `slack_summary_settings_test.dart` updated for the new navigation (tap "Slack" to reach the fields, no more inline "Morning summary (Slack)" heading on the top-level screen) — all 3 cases pass. `settings_export_main.dart` (the only `*_main.dart` scaffold that actually used `debugAutoTriggerExport`) repointed at the new `BackupSettingsScreen`; the other three `*_main.dart` files use plain `SettingsScreen()`/`SettingsScreen(debugAutoTriggerImport: ...)`-free construction and needed no change. Grepped for any other reference to the removed private `_SettingsPanel`/`_ManageLinkRow`/`_CalendarSettingsPanel` classes — none found outside the old file.

## [2026-09-10] Left-edge time badge extended to resize (top AND bottom), always left-pinned

Requested directly, as a follow-up correction to the same-day move-drag indicator: "the time in accent color bg when moving or rezising, sohul always be on left[;] also when resizing... we should show times not only when wiggling." The previous session's fix only covered move-drag (`_isDragging`); resize left the badge to wiggle's own `_edgeTimeLabels`, which are explicitly SUPPRESSED during an active resize (`wiggleEnabled` is false whenever `_isResizing`/`_isResizingTop`) — so mid-resize showed no live time at all, only the pill's own inline duration text.

Extended the same `_buildSplit` block (`timeline_screen.dart`) to also render during `_isResizing`/`_isResizingTop`, always pinned to the same left spot over the hour gutter (`left: -widget.left`, unchanged) rather than following the resize handle to the right/bottom edge. Which edge's TIME is shown depends on which edge is actually moving: bottom-resize (`_isResizing`) shows the live END (`task.scheduledAt + _previewDurationMinutes`, vertically pinned at the pill's own bottom via `top: _pillHeight(...)`); top-resize and move both show the live START (`_previewTopStartDelta`-adjusted or `_previewStartsAt` respectively, pinned at `top: 0`) — the edge that's actually moving in each case, mirroring `_edgeTimeLabels`' own existing start/end split.

**Checks:** `flutter analyze`/`dart format .` clean (same 2 pre-existing unrelated warnings). Extended `drag_move_time_label_test.dart` with 3 new cases (bottom-resize shows live END, top-resize shows live START, release-then-still-armed correctly shows wiggle's OWN pair taking over rather than nothing) — all confirmed to fail against the pre-fix code first. Hit and fixed a real test-authoring bug along the way, not a production bug: a single large `gesture.moveBy(Offset(0, 40))` call was silently landing INSIDE the vertical-drag recognizer's unresolved gesture-arena window and never firing `onDragUpdate` at all — `_resizeOffset`/`_resizeTopOffset` stayed exactly 0 despite the drag visually "running," which the original loose `greaterThan(x - 5)` assertions didn't catch (a zero delta still passed the tolerance window). Fixed by splitting into 8 small 5px moves with a `pump()` between each, matching how a real per-frame drag reports incremental deltas — same fix applied retroactively to the original move-drag test's own assertion, tightened to check the exact time changed rather than just checking the label exists. Reran `armed_edit_task_test.dart`, `task_edit_time_labels_test.dart`, `task_edge_time_label_test.dart`, `pending_task_pill_test.dart` individually — all pass; `multi_task_group_resize_test.dart`'s 4 pre-existing failures confirmed via `git stash` to fail identically on unmodified `main` (a similar single-big-`moveBy`/gesture-arena issue in that file's own harness), unrelated to this change and left unfixed as out of scope.

## [2026-09-10] Left-edge time badges: wiggle corrected to the gutter, move now shows both edges, zones get the same treatment

Three corrections requested directly in one message, all extending the same left-pinned accent-time mechanism from the two prior sessions: (1) "when wiggling on left generally all cases on left side" — Edit Mode's resting wiggle labels (`_edgeTimeLabels`) had NEVER actually been on the gutter; they sat at the pill's own top/bottom-right (`left: 0, right: 0` inside the row, right-aligned), a leftover from before the gutter-pinning trick existed. (2) "when moving also end should be shown in blue atm only beginning start time" — a move-drag showed only the live start, dropping the end entirely, unlike a resize (which correctly shows just the one moving edge) or wiggle (which shows both at rest). (3) "also should be show for zones" — zones had no live time badge at all during their own move/resize.

**Task side** (`timeline_screen.dart`): `_edgeTimeLabels` (the resting wiggle pair) switched from `left: 0, right: 0` to `left: -widget.left` — the exact same gutter-pinning trick the move/resize badge already used, making wiggle's own labels land in the same spot rather than at the pill's far edge. The old standalone move/resize badge block was folded into a new `_liveEdgeTimeLabels` method (mirroring `_edgeTimeLabels`' own shape) which now returns TWO labels for a move (start AND end, both shifting by the same live offset since duration is unchanged) and ONE for a resize (only the edge actually moving, unchanged from before). Both `_buildSplit` and the (dead-code, but kept consistent) combined-layout branch now choose between `_liveEdgeTimeLabels` (during an active gesture) and `_edgeTimeLabels` (at rest, while wiggling) via one `if (_isDragging || _isResizing || _isResizingTop) ... else if (wiggleEnabled) ...` — the two were previously two separate, independently-gated blocks that could theoretically both fire at once.

**Zone side** (new): `ZoneBackgroundBlock` gained `liveStartMinutes`/`liveEndMinutes` (nullable, independent — a resize sets only the moving one) and renders the identical `TaskEdgeTimeLabel` pair, gutter-pinned via `-(left - zoneBackgroundOffset)` (the block's own known offset from the day column's true x=0, same derivation as tasks' `-widget.left`). `_DraggableZoneBlock` computes these from its own existing `_moveMinutesDelta`/`_resizeMinutesDelta`/`_resizingTopEdge` state — no new gesture plumbing needed, since zone move/resize already existed from an earlier session; this only adds a visual readout of state that was already being tracked.

**Checks:** `flutter analyze`/`dart format .` clean project-wide (same 2 pre-existing unrelated warnings). `task_edit_time_labels_test.dart`'s wiggle test gained a real position assertion (previously checked count/time only, which couldn't have caught the wrong-side bug) — confirmed it actually fails against the pre-fix right-aligned layout first; getting this assertion right took two attempts, since comparing `Positioned` widgets' own `getTopLeft` is meaningless when both left- and right-aligned content share the same outer box coordinates — fixed by measuring the rendered time TEXT's position instead. `drag_move_time_label_test.dart`'s move-drag tests updated for two labels instead of one, including a check that both shift by exactly the task's own duration apart. New zone tests in `zone_task_view_move_resize_test.dart` (3 cases: move shows both edges, bottom-resize shows only the live end, idle shows nothing) — hit and fixed a real test-hang: calling `onMoveEnd`/`onResizeBottomEnd` (both trigger real, unawaited repository I/O via `_commitMove`/the cascade path) outside `tester.runAsync` hung the whole file indefinitely, matching this file's own established `runAsync`-wrapping convention for every other commit call, which the new tests had missed. Also fixed a real production bug caught by the new zone tests before they were "confirmed passing": the first version of `ZoneBackgroundBlock`'s live-label logic rendered BOTH edges during a resize (start and end always non-null), when only the moving edge should show — fixed by passing `null` for the anchored edge, matching tasks' own contract exactly.

## [2026-09-10] Timeline view-cycle toggle swap, hide-imported-tasks widened to Zone view, "Show time" default off, and Zone view row polish

Six requests bundled in one message. **View cycle**: `DevZoneViewInCycle` (debug-only, could remove Zone view from the Task→List→Zone cycle) is replaced entirely by `DevListViewInCycle` — "zone view and task view are on always, and list view as default off." Zone view's reachability now depends purely on `FeatureFlags.zoneEnabled` (confirmed via AskUserQuestion: no separate dev override left for it at all); Task view was already never skippable. List view is the only mode `TimelineViewMode.next()` can now skip, gated by the new toggle, defaulting to **false** (excluded by default) — the opposite of "on by default" every other dev toggle in this file uses, since exclusion is the new shipped behavior here. `day_strip.dart`'s `next()` gained a second required param (`listViewEnabled`) alongside the existing `zoneFeatureEnabled`; `timeline_screen.dart`'s own separate `zoneViewEnabled` gate simplified to drop its now-removed dev-toggle term.

**Hide imported tasks**: `DevTimelineListOnlyAmbleTasks` renamed to `DevHideImportedTasks` (same default-false semantics, described from the opposite direction per direct request) and widened from List-view-only to also filter Zone view's `ZoneDayTimeline.externalEvents` — previously Task view and Zone view were both explicitly unaffected; now only Task view is. The Developer settings label changed from "Only Amble tasks (List view)" to "Hide imported tasks (List + Zone view)".

**Show time default**: `DevTimelineTaskTimeRangeVisible`'s default flipped from `true` to `false` — "show time off as default."

**Zone view row polish**, all in `zone_container_block.dart`: (1) the header's `"${zone.title} (${duration})"` lost its parentheses, becoming `"${zone.title} ${duration}"` (confirmed via AskUserQuestion: stays in the same position, just without the parens, rather than relocating next to the time range). (2) the in-zone task row's category badge grew from `theme.sizeTaskBadge` (20px) to `theme.sizeTaskBadgeXl` (32px, the same token the Appearance → Task size "Large" option uses, confirmed via AskUserQuestion) — "tasks pill (circle with icon large." (3) the gap between that badge and the task title grew from `theme.spacingXs` to `theme.spacingSm`, matching the gap `TaskCapsuleBlock` already uses between its own pill and title outside a zone — "add larger gap between 'pill' and name in tasks inside zones... same gap as in tasks outside zones."

**Checks:** `flutter analyze`/`dart format .` clean project-wide (same 2 pre-existing unrelated warnings). Regenerated `dev_config.g.dart` for the two renamed Riverpod classes. New `day_strip_view_cycle_test.dart` (2 cases: List view excluded by default never appears across 6 cycle taps; included via the dev toggle, it does) — needed a real Hive-backed `preferencesRepositoryProvider` override, since `ShowHourLabelsSetting`/`ZoneViewEnabledSetting` throw without one; both confirmed to fail against a temporarily-reverted `next()`. `list_mode_task_filters_test.dart` gained a `zoneViewEnabled` pump param and 2 new cases proving the widened toggle actually reaches Zone view (both confirmed failing pre-fix). `zone_container_block_test.dart` gained a real `Category` fixture (previously every test used an empty `categoriesById`, which meant no test ever exercised the badge/emoji path at all) and 2 new cases for the badge size and gap (both confirmed failing pre-fix) — the gap assertion needed a second attempt, since measuring the emoji `Text` widget's own bounds directly overstates the gap by its font-metric padding; fixed by measuring the badge `Container`'s edge instead. Fixed 3 real, expected test breakages from the bracket removal: `zone_container_block_test.dart` and `zone_day_timeline_list_test.dart` both had hardcoded `'Morning ritual (Nh)'` strings needing the parens dropped.

## [2026-09-10] Tracked behavior "Unit of measure": Distance, Reps, Count, Custom

Requested directly: "when we create a behavior... we add new option Unit of measure: Time, Distance, Reps, Count, Custom > opens small sheet with entry field (Label, Unit (placeholder, times, km, hours, glasses)." `BehaviorTargetType` gained `distance` (fixed unit "km"), `reps` ("reps"), and `custom` (user-defined label/unit pair); the existing `binary` ("Did it") stays as a 6th chip alongside the four measured units. See docs/DECISIONS.md's own entry for the full reasoning, including a real mid-session correction: the first pass wrongly collapsed "Reps" and "Count" into one relabeled chip before being caught and split into two distinct variants (`count`'s own default unit changed from "reps" to "times" as a result — a real, if small, behavior change for any existing Count-type row's displayed text).

New `custom_unit_sheet.dart` — a genuinely small (`AppSheetSize.small`), content-sized sheet with a Label field and a Unit field (raw `TextField`, hint text "e.g. times, km, hours, glasses"), opened by tapping the "Custom" chip. `TrackedBehavior` gained `customUnitLabel`/`customUnitName` (both nullable, both required together whenever `targetType == custom`, enforced by a new constructor assert). The Custom chip's own displayed label switches to the unit's saved Label once set (e.g. "Water" instead of generic "Custom"), and re-tapping it reopens the sheet pre-filled for editing.

**Checks:** `flutter analyze` clean, `dart run build_runner build` regenerated both affected `.g.dart` files cleanly (new Hive field indices 3/4/5 on `BehaviorTargetType`, 7/8 on `TrackedBehavior`, all additive — no existing stored value's meaning changes). New `tracked_behavior_unit_of_measure_test.dart` (5 cases) plus a fix to `tracked_behavior_list_screen_test.dart`'s now-outdated Count-unit expectation, plus a new sibling case for Reps — full `test/features/tracked_behavior/` suite (30 tests) passes. Hit and fixed a real test-hang along the way: tapping the real form's "Save" does genuine unawaited repository I/O and must run inside `tester.runAsync`, the same class of bug found in an earlier session's zone tests — the new test's first draft omitted this and hit the 10-minute per-test timeout before being fixed.

## [2026-09-10] Tracked behavior cards redesigned: weekly/monthly/six-monthly completion views, with a global view switcher

Requested directly, from a mockup: "let us build these views on the tracked section... this is replacing the current cards... in the same way like in timeline we have view switcher, in the same positions there will be view switcher for each of these here, weekly, monthly, and six monthly... tapping on the card opens edit task for now." Confirmed via AskUserQuestion, three scoping decisions before building: (1) the mockup's grouped-by-section outer layout (collapsible "Morning"/"Health" headers with icons and counts) does NOT exist in this codebase and was explicitly descoped — only `TrackedBehaviorRow`'s own card content changes, `TrackedBehaviorListScreen`'s existing flat list stays as-is; (2) the view switcher is ONE global control for the whole screen (mirroring `TimelineViewMode`'s own single button), not per-card; (3) tapping a card already opened `showTrackedBehaviorForm` for editing before this session — genuinely unchanged, not new wiring.

**New completion-history derivation** (`behavior_completion.dart`): there was no completion log for tracked behaviors at all — `completedDaysFor` derives it from `Task` rows instead, treating any `Task` with `behaviorId == behavior.id` and `status == TaskStatus.completed` as marking its `scheduledAt` day "done." A day cell's own two visual states, confirmed via AskUserQuestion: solid accent fill = completed that day, accent outline = today (the two are independent — today can be both filled and outlined, or just outlined if not yet done).

**Three card variants** (`_WeeklyRow`/`_MonthlyGrid`/`_SixMonthlyHeatmap` in the rewritten `tracked_behavior_row.dart`), sharing one `_DayCell` widget: Weekly is a Mon-Sun 7-cell row with weekday initials underneath; Monthly is a plain WRAPPING grid of the current month's numbered day cells — confirmed via AskUserQuestion as simpler than a real weekday-aligned calendar grid, so days 1..N just flow left-to-right with no blank leading cells; Six-monthly is a GitHub-heatmap-style grid of small unlabeled day squares spanning the trailing ~183 days, wrapping into rows (confirmed via AskUserQuestion, over a single horizontally-scrollable row) with month name labels floated loosely above roughly where each month's squares begin — "same as day and week grid of squares that don't distinguish when month ends when new starts," i.e. the squares themselves flow continuously with no visual month-block separation, only the label row above marks where a month's squares roughly begin.

**New persisted setting**: `TrackedBehaviorViewMode` (own `@HiveType(typeId: 13)` enum, weekly/monthly/sixMonthly — the next free typeId, confirmed by checking every existing `@HiveType` declaration) + `TrackedBehaviorViewModeSetting` (mirrors `TaskSizeSetting`'s exact shape), defaulting to weekly. The switcher itself is a plain cycling `IconButton` in `AppBottomExtensionBar`'s `leading` slot — the exact same slot and interaction pattern `DayStrip`'s own Zone/List/Task view-cycle button already uses, previously `SizedBox.shrink()` on this screen since it had no view of its own before now.

**Checks:** `flutter analyze` clean project-wide (same 2 pre-existing unrelated warnings). `dart run build_runner build` regenerated the new enum's Hive adapter and the new Riverpod provider cleanly; `hive_registrar.g.dart` picked up the registration automatically. Fixed a real, expected breakage: `tracked_behavior_list_screen_test.dart` never overrode `preferencesRepositoryProvider` (the old screen had no dependency on it) — now throws `HiveError: Box not found` without a real Hive-backed override, fixed by adding one plus a `taskRepositoryProvider` override (the new card also reads `taskListProvider` for completion derivation). New `tracked_behavior_view_mode_test.dart` (5 cases: default view, full 3-step cycle, real-persistence-across-rebuild, a completed linked task actually fills a cell while an uncompleted one shows nothing, and a focused unit test on `completedDaysFor`'s own filtering logic) — all confirmed to fail against pre-fix code first. Hit two real test-authoring bugs along the way: (1) tapping the switcher does genuine unawaited `PreferencesRepository`/Hive I/O and hung `pump`/`pumpAndSettle` indefinitely until wrapped in `tester.runAsync` — the same class of bug hit repeatedly earlier this session; (2) a bare `find.byType(Container)` scan for "any cell filled with the accent color" produced a false positive from the bottom bar's own unrelated "+" `AppIconButton` (which also fills with `colorAccent` for its own press-feedback circle) — fixed by scoping the search to descendants of `TrackedBehaviorRow` specifically. Full `test/features/tracked_behavior/` suite (35 tests across 6 files) passes.

## [2026-09-10] Zone container cards de-tinted to the plain card surface

Requested directly: "the zone card make the color of the card [same as] on manage... as card on tracked... non colored." `ZoneContainerBlock` (Zone view's own container card on the Timeline) and `ZoneBackgroundBlock` (the equivalent fill on Task view — the two were already deliberately kept visually consistent with each other, per a prior decision) both filled with `theme.colorZoneBackground`, a distinct zone-tint token, unlike the plain `theme.colorSurfaceSecondary` the Zone list row (Settings → Manage → Zones) and the Tracked behavior card both already use. Confirmed via AskUserQuestion which card was meant (this exact pair, not the Manage row itself, which was already correct) and that the fix should apply to both Zone view and Task view together, not just one. Both switched to `colorSurfaceSecondary`; `colorZoneBackground` itself is untouched and still used elsewhere (a settings-screen divider, `external_event_block.dart`'s own fill).

**Checks:** `flutter analyze` clean (same 2 pre-existing unrelated warnings). No test asserted the old color directly; reran `zone_container_block_test.dart` (15), `zone_task_view_move_resize_test.dart` (8), and `zone_day_timeline_list_test.dart` (15) — all pass, no regressions.

## [2026-09-11] Zone card "flat style" dev toggle — strips background and padding

Requested directly: "add a control in admin that affects the grid zone view and removes the background from zones and[,] uh, padding as well[;] so what's left is a title... and underneath the tasks related inside this zone[,] and duration... also stays." New `DevZoneCardFlat` toggle (`dev_config.dart`, default off) in the Developer settings page's existing "Zone view" section — zone view (grid) only, threaded through `ZoneDayTimeline`/`TimelineScreen` as a plain `flatStyle` bool, the same "read the provider once at the top, pass a plain field down" pattern every other dev toggle reaching `ZoneContainerBlock` already uses (that widget is a bare `StatelessWidget`, no Riverpod access of its own).

`ZoneContainerBlock`'s own `DecoratedBox` fill (`color`) and its `Padding` both become conditional on `flatStyle`: color turns `null` (no fill at all, not just transparent) and padding collapses to `EdgeInsets.zero`, leaving the title/duration header directly above a bare row list with no card chrome around either. The drop-target border (shown only mid-drag, when a task is about to be dropped into this zone) is deliberately NOT touched by this toggle — it's functionally meaningful feedback, not decorative chrome, so it still draws even in flat style. Task view's `ZoneBackgroundBlock` (the purely decorative fill there) is unaffected — the request named specifically "the grid zone view," and that widget isn't a real layout container the way `ZoneContainerBlock` is.

**Checks:** `flutter analyze` clean (same 2 pre-existing unrelated warnings). `dart run build_runner build` regenerated the new provider cleanly. New cases in `zone_container_block_test.dart` (off keeps fill+padding; on removes both while title/duration/member-task-row all still render) — both confirmed to fail against pre-fix code first. Reran `zone_day_timeline_list_test.dart` (15) and `zone_task_view_move_resize_test.dart` (8) — no regressions.

## [2026-09-11] Task view's left hour labels right-aligned, closing the ragged gap before pills

Requested directly: "on task view time on left too big padding[;] make same as on right hand side." Traced the actual geometry: task content starts 80px from the screen's left edge (24px `spacingScreenPadding` + 56px `_hourGutterWidth`, reserved for the widest label, "12:00 PM") but only 24px from the right edge (`rightEdgeInset`, plain `spacingScreenPadding`). Confirmed via AskUserQuestion that matching the two exactly (80px → 24px) would clip "12:00 PM" against the first pill, since the label text itself needs close to the full 56px — so the fix isn't shrinking the reserved gutter.

The real cause of the reported "too big padding": every label was left-aligned at the screen edge inside that fixed 56px column, so a SHORT label ("9 AM") left a large, ragged gap before the pill column while only the widest label ("12:00 PM") filled it. Confirmed via AskUserQuestion: right-align every label within its column instead, so all of them sit flush against the pill edge regardless of their own text width — closing the visual gap for short labels without touching the reserved width at all, and with zero risk of clipping the widest one.

`TaskBoundaryMarkers` (`task_boundary_markers.dart`) gained an optional `columnWidth` param — when set, each label's `Positioned` gets that fixed `width` and an `Align(alignment: Alignment.centerRight)` wrapper instead of the previous bare left-anchored `Text`; null (the default, for any other caller) keeps the original left-aligned behavior unchanged. `timeline_screen.dart`'s own call site now passes `columnWidth: _hourGutterWidth` — the exact same constant that already sized the reserved space, just now used to align within it as well as measure it.

**Checks:** `flutter analyze` clean (same 2 pre-existing unrelated warnings). New cases in `task_boundary_markers_test.dart`: `columnWidth` set right-aligns (checked via the `Align` widget's own `alignment` property, not measured pixel position — an initial pixel-based assertion passed even against a temporarily-reverted, still-left-aligned version, because `flutter test`'s fallback font happens to render "1:00 AM" wide enough to fill the whole 56px column regardless of alignment; fixed by asserting the structural property directly instead) and unchanged left-alignment when `columnWidth` is left null. Both confirmed to fail against pre-fix code, including a second confirmation after the assertion itself was corrected. Full existing `task_boundary_markers_test.dart` suite (9 cases) and a spot-check of `list_mode_task_filters_test.dart`/`entrance_scale_test.dart` all pass, no regressions.

## [2026-09-11] Zone flat style extended: bigger zone-to-zone gap, per-task cards

Two follow-ups to the same-session "flat style" toggle (`DevZoneCardFlat`), requested directly: "add gap between zones zone view (flat style)... tasks should be in 'card' like they are in manage (but keep size of them as they are in zone view atm)."

**Zone-to-zone gap**: `ZoneDayTimeline`'s `ListView.separated` gap between rows was a flat `theme.spacingXs` (4px) regardless of style — fine in normal style, where each zone's own card edges already visually separate it from its neighbor, but too tight in flat style once that card chrome is gone. Confirmed via AskUserQuestion: the separator now checks `devZoneCardFlat` and uses `theme.spacingMd` in flat style, leaving normal style's 4px untouched.

**Per-task cards**: `_ZoneTaskRow` (`zone_container_block.dart`) gained a `flatStyle` param, threaded from `ZoneContainerBlock`'s own existing field. When true, the row wraps its unchanged `Row` content in a `DecoratedBox` using the exact same "Manage" card style already confirmed for `ZoneContainerBlock` itself (`theme.colorSurfaceSecondary` fill, `theme.radiusXl` corners) — matching `ZoneListScreen`'s own `_ZoneRow`. Confirmed via AskUserQuestion on two points: (1) the card adds NO internal padding of its own (Manage's own `spacingMd` padding would compress the badge/title inside the row's existing fixed `zoneContainerRowHeight`, 44px, which must stay exactly as-is — "keep size of them as they are in zone view atm"); (2) the card only applies in flat style, since normal style already has the zone's own container card as the visual wrapper and a second nested card there would be redundant. `_ZoneExternalEventRow` was deliberately left untouched — the request named "tasks" specifically.

**Checks:** `flutter analyze` clean (same 2 pre-existing unrelated warnings). New cases in `zone_day_timeline_list_test.dart` (flat style gets the larger `spacingMd` gap) and `zone_container_block_test.dart` (flat style gives the task row its own colored/rounded `DecoratedBox` with the row's `SizedBox` height still exactly `zoneContainerRowHeight`; normal style's task row stays a plain colorless `BoxDecoration`, unchanged) — all three new cases confirmed to fail against pre-fix code first. Full `zone_container_block_test.dart` (19) and `zone_day_timeline_list_test.dart` (16) suites pass, no regressions.

## 2026-09-12 — Haptic feedback layer

Requested directly: "Add haptic response to interactions." The app had no haptic code at all before this.

**Shipped:**
- `lib/core/haptics.dart` — `AmbleHaptic` intention enum (`selection`/`tap`/`lift`/`drop`/`warning`/`success`), a `Haptics` interface, `PlatformHaptics` (fire-and-forget, swallows platform errors so a missing motor can never break a tap) and `SilentHaptics`. No new dependency; Flutter's built-in `HapticFeedback` covers all six.
- `lib/core/haptics_provider.dart` — `hapticsProvider`, keepAlive.
- `android/app/src/main/AndroidManifest.xml` — added the missing `VIBRATE` permission. Without it every Android haptic silently no-ops.
- `AppPressFeedback` — fires on press-down. This is the highest-leverage wiring point: 13 files route through it, covering `AppButton`, `AppIconButton`, `AppSelectableChip`, `AppMicButton`, `AppFieldActionButton` and every card/row, with no change at any of those call sites.
- `AppSwitch` — Material branch only; `CupertinoSwitch` already plays its own on iOS.
- `CompletionCheckbox` — `success` on complete, the lighter `selection` on undo.
- `timeline_screen.dart` — task move drag (lift / per-snap-increment tick / drop), task top- and bottom-resize (lift/drop), zone move and both zone resize edges (lift/drop), long-press-to-arm (`lift`), Edit Mode entering/leaving (hooked to the existing `ref.listen` on `editModeEnabledProvider`, so both the two-finger gesture and `_EditModeLink` are covered by one point), and `warning` when a drag arms the delete target.
- `place_task_line.dart`'s `decorationOnly` layer opted out with `haptic: null` — it spans the whole empty day column and would otherwise buzz on the first touch of every scroll.

**Tests:** `test/core/recording_haptics.dart` (recording fake), `test/core/haptics_test.dart` (7 cases), `test/features/timeline/drag_haptics_test.dart` (2 cases, including one that holds the finger still for ten frames to prove the snap tick is edge-triggered rather than per-frame). Every assertion was confirmed to fail against reverted production code before being accepted — including a run against a deliberately naive per-frame snap tick, which the edge-detection test caught.

**Verification:** `flutter analyze` clean (only the 2 pre-existing `modalTitle` warnings in the untouched `task_detail_sheet.dart`). `dart format` clean. New tests all pass.

**Open / unverified:** the actual feel is unverified — `flutter test` has no vibration motor and `HapticFeedback` is inert there, so the tests prove only that the correct intention fires, never how it feels. Strength tuning and the Android mapping both need a real-hardware pass. Haptics is also absent from `SCOPE.md`; built on direct request and flagged in `DECISIONS.md` rather than silently adopted.

**Pre-existing failures found (not caused by the haptics work, not fixed):** the full suite finishes at 1012 passing / 6 failing, plus the separately-known `list_mode_clustering_test.dart` hang (which times out at the 10-minute ceiling, as CLAUDE.md already records for this directory). The 6:

- `multi_task_group_move_test.dart` (1) and `multi_task_group_resize_test.dart` (3) — a selected task's id is missing from `editSelectionProvider` when the group drag commits. Verified independent of this change by stripping the Edit Mode haptic out of `ref.listen(editModeEnabledProvider, ...)` and re-running: identical failures. `multi_task_selection_test.dart` (12 cases, including "exiting Edit Mode clears the selection" — the very callback the haptic hooks into) passes fully, so the selection provider itself is sound.
- `place_task_line_test.dart` (1) — a long-press no longer renders its "11:00 AM" time label. This one is in a file the haptics work DID touch, so it was checked directly rather than assumed: reverting the `haptic: null` line and re-running reproduces it exactly, so it is pre-existing.
- `tap_empty_space_quick_create_test.dart` (1) — an ambiguous finder ("Found 2 widgets with text 'Existing task'"), a test-scoping problem rather than a behavior one.

All six belong to this session's earlier multi-task / zone-view / task-text-column work and need their own pass.

## 2026-09-12 — Hour-gutter overflow fixed; bottom-bar "misalignment" disproved

**1. Hour labels ran under the task pills — real, fixed.** Reported from a device screenshot. The labels right-align inside a fixed column that ends exactly where task pills begin, so a label wider than its column doesn't clip — it paints straight into the pill column. Measured with the real bundled JetBrains Mono: "11:00 AM"/"12:00 PM" are 57.6px in a 56px column, while "1:00 PM" is 50.4 and fits. That's precisely why only the 10-12 o'clock rows collided. `_hourGutterWidth` 56 → 72, clearing the widest label with ~14px to spare.

**A font-measurement trap worth recording.** The first measurement said "11:00 AM" needed 96px and every label overflowed — which would have led to a 96px+ gutter. That was `flutter test`'s fallback font, not JetBrains Mono. docs/ERROR_LOG.md already records a pixel assertion that passed against reverted code for this same reason. Added `test/support/load_app_fonts.dart` so any test measuring rendered text width loads the real bundled font first; tuning against the fallback would have sized the column for a font no user ever sees.

**2. "The Timeline bottom sheet sits higher than Inbox/Tracked" — could not reproduce; no change shipped.** The hypothesis was that `DayStrip` (Timeline-only) pushes the bar up, so the day tiles were shrunk 64→52 with matching padding. Measuring afterwards showed this had **no effect on bar height**: `AppBottomExtensionBar` renders 76.0px on Timeline and 76.0px with a bare-IconButton leading (Inbox/Tracked's shape) — identical. The binding constraint is the 60px `AppIconButton` (`spacingXl * 1.5`) plus the bar's own padding, not the day tiles, which at 52px no longer reach the floor. All three tabs share the same bar widget and the same nav bar. The tile/padding edits were reverted rather than shipped as a no-op change. If the difference is real on device it comes from somewhere else — worth a fresh look with an on-device measurement rather than more guessing at the widget tree.

**Verification:** new `hour_gutter_overflow_test.dart` (4 cases: no label overflows, every label keeps ≥8px before the pills, a source-reading guard that fails if `_hourGutterWidth` drifts from what the tests assert, and one pinning the original 56px defect). Both overflow cases confirmed to fail at 56 and pass at 72. Timeline suite 347 passing / 5 failing — all 5 from the known pre-existing set, one fewer than the prior baseline. `flutter analyze` clean apart from the 2 pre-existing `modalTitle` warnings.

## 2026-09-12 — Zone-view completion checkbox pinned right

Reported directly from a device screenshot: "the position of the checkbox in the zone view should be on the right hand side. Currently, it's somewhere in the middle."

**Cause (measured, not guessed).** `_ZoneTaskRow` laid its children out as time `Flexible(flex: 2)` → title `Expanded(flex: 3)` → checkbox. With no gap widget, the checkbox simply followed wherever the title's box ended, so its position tracked title length: measured on a 400px row, a long title ("Deep work") left it 3.75px from the row edge while a short one ("cf") stranded it **103.5px** short. That short-title case is exactly what the screenshot shows.

**Fix.** Title `Expanded` → `Flexible` (so it takes only the width its text needs, still ellipsizing when tight), plus a trailing `Spacer(flex: 100)` to absorb the remainder.

**The flex weight is load-bearing, and the reason is worth recording.** A `Spacer` *is* an `Expanded`. At its default `flex: 1` it competed with the time (2) and title (3) columns and won only 1/6 of the slack — the short-title case still failed. Only outweighing both columns hands it effectively all the remaining width, making the checkbox land identically regardless of title length.

**Two wrong turns, both caught by measuring rather than by eye:**
- `Flexible(flex: 0)` on the time column collapsed it to zero width and broke 14 tests.
- `IntrinsicWidth` around the time column changed nothing — the column was never the problem; probing at an 800px row width had shown it ballooning to 221px for an ~88px string, which looked like the cause but was a symptom of the same missing gap.

**Task (spatial) view: no change, by design.** Its checkbox is already flush against the text column's right edge and vertically centred on the title (measured: checkbox [352, 400] in a 400px row; title vertical [0,18], checkbox [0,48] — a 48px tap target centred on an 18px text line). It reads as inset only because `textColumnRight` reserves `_zoneLabelGutterWidth` (24px) whenever zones exist, to clear the rotated "Work"/"Commute" labels down the right edge — visible in the same screenshot. Confirmed with the user as correct and deliberately left alone.

**Verification:** 2 new cases in `zone_container_block_test.dart` — one asserting the checkbox is flush right, one asserting a SHORT title leaves it at the same right edge (the second is what distinguishes a genuinely right-anchored layout from one that merely looks right for a single title length). Full file 21/21. Timeline suite 353 passing / 1 failing, that one the known pre-existing `place_task_line_test.dart` case. `flutter analyze` clean apart from the 2 pre-existing `modalTitle` warnings.

## 2026-09-12 — Dark ramp re-anchored from specified values; nav + day-strip unified as one floating pane; day-strip jump fixed

**1. Dark ramp re-anchored.** Body given as `#121110`, zone/card as `#171615`. Converted back through the project's own OKLCH pipeline: `oklch(0.178, 0.0026, 67.7)` and `oklch(0.201, 0.0025, 67.7)`. Two real changes versus the old ramp — everything slightly darker, and **chroma dropped ~3x** (0.008 → 0.0026), so dark mode now reads as near-neutral charcoal instead of warm brown-grey. The ramp was extended upward from those two anchors with widening steps (0.023 → 0.031 → 0.038), preserving the existing rationale that perceived separation compresses near black. New `ink650` (0.270) is the floating-overlay rung, kept distinct from `ink600` which is the hairline tone.

**Contrast re-verified rather than assumed** — a darker base could have broken text. It didn't: primary text 15.83:1 (was 15.63), secondary 7.61:1 (was 7.52), and every dark category swatch still clears AA at ≥4.79:1. All slightly *better*, since the background moved away from the foreground.

**2. Light-mode equivalents derived.** `cream0/1/2` + `creamOverlay` mirror the dark anchors' *relationships* rather than their values: dark steps up from its base (0.178→0.201→0.232), light steps down from its own (0.974→0.951→0.920), both carrying the same faint 0.0026 chroma at hue 67.7 so the themes read as one product. Deliberately **additive** — `surface0`-`surface3` are untouched, since they carry the existing true-grey spec across many roles (panes, fields, focus states) and re-tinting the whole scale would have been a far wider change than requested.

**3. Nav + adjacent pane unified.** `AppBottomExtensionBar` (the day-strip/calendar pane) was a top-rounded slab welded to the screen edge painting `colorSurfacePrimary`. It's now a floating pane matching the nav: `colorSurfaceOverlay` fill, `radiusXl` on all four corners, and it owns its own side insets so all three call sites (Timeline, Inbox, Tracked) align without each knowing the measurements. Its bottom `SafeArea` is disabled — the nav below owns that inset for the stacked unit, or the two halves would separate by the inset's height. **Borders removed from both halves** per direct request; in dark mode the overlay is already the lightest step in the ramp, and that lightness difference is what separates it from the page.

**4. The day-strip jump — found and fixed.** The "return to today" chevron was conditionally inserted into the strip's `Row` (`if (_scrolledAwayFromToday)`), so every time the user scrolled across today's date the Row's child count changed and the day tiles shifted sideways by the chevron's full width. Its slot is now always in the layout, with only opacity animating, wrapped in `IgnorePointer` so a transparent chevron is never tappable.

**Verification:** 2 new cases in `day_strip_stability_test.dart`, both confirmed to fail against the old conditional-insertion code. A third test was written and then **deleted rather than kept** — it had early-return escape hatches for date-dependent tile finders, so it could pass by doing nothing, which is worse than no test. Token tests 29/29. Broad sweep 666 passing / 1 failing (`task_providers_test.dart` recurrence case — fails in isolation too, in code untouched here, pre-existing). Timeline 354 passing / 2 failing, both the known pre-existing pair. `flutter analyze` clean apart from the 2 pre-existing `modalTitle` warnings.

**Unverified:** every colour judgment here is measured, not seen. Whether the new near-neutral dark ramp and the cream light base actually look right — and whether the unified pane reads as one element on a real screen — needs a device pass.

## 2026-09-12 — Zone task names restored (self-inflicted regression), hour labels left-aligned, bottom panes joined

**1. Task names invisible in Zone view — a regression I introduced, now fixed.** The previous session's checkbox fix paired the title's `Flexible(flex: 3)` with a `Spacer(flex: 100)` to pin the checkbox right. That worked for the checkbox and starved the title: measured at **7.5px wide**, so every task name collapsed to an ellipsis. Replaced with a plain `Expanded` title and no Spacer — one widget both left-aligns the title and pushes the checkbox to the edge.

**Why the existing tests missed it:** all three checkbox-position tests passed straight through the regression, because none of them looked at the title. Added two that do — one asserting the title gets real width (>60px and >20% of the row), one asserting a long title ellipsizes instead of shoving the checkbox out. The first is confirmed to fail against the shipped regression.

**2. Hour labels switched back to left-aligned.** Requested: "same padding as zone names on the other side, left aligned." The rotated zone names sit at `spacingScreenPadding` from the right edge; the hour labels now sit at the same inset from the left, so the two annotations frame the day symmetrically. Implemented by dropping `columnWidth` from the call site — that parameter is what switches `TaskBoundaryMarkers` to right-aligned, which an earlier request had asked for and this one reverses. The widget's own `columnWidth` tests still pass and stay valid; a new call-site test pins the screen's choice.

**3 + 4. Nav and adjacent pane joined into one object, no bottom margin.** Both halves dropped their `spacingSm` bottom inset (that gap was both the seam between them and the margin under the nav). Corners are now complementary — extension pane rounds only its top pair, nav only its bottom — so they meet on a shared square seam instead of reading as two stacked pills. The extension pane also drops its own `boxShadow`: `shadowPane` offsets downward (+2y), so it would have cast onto the nav and drawn a visible line through the middle of the combined pane; the nav carries the shadow for the whole unit.

**Verification:** `connected_bottom_pane_test.dart` (4 cases: no bottom gap, top-only corners, no shadow, no border in either theme). `zone_container_block_test.dart` 23/23. Timeline + core sweep 480 passing / 2 failing — both the known pre-existing pair (`place_task_line`, and the `multi_task_group_resize` case that passes in isolation). `flutter analyze` clean apart from the 2 pre-existing `modalTitle` warnings.

## 2026-09-12 — Checkbox alignment (both views), light-mode surfaces deepened, full light palette re-solved

**1. Zone-view checkbox regression fixed.** A nested task row's checkbox sat 40px from the screen edge (24 page padding + 16 zone-card padding) while a standalone task row's sat at 24px — a visible misalignment down the column. The zone card's right padding is now dropped (`EdgeInsets.fromLTRB(md, md, 0, md)`), so nested and standalone checkboxes share one edge. Measured before and after; the existing "keeps the card's padding" test was updated to the asymmetric intent rather than deleted.

**2. Task-view checkbox was genuinely misaligned — my earlier "it's centred" claim was wrong.** Measured: title centre at y=9, checkbox centre at y=24 — **15px apart**. The 48px tap target was defining the row height under `CrossAxisAlignment.start` while the text sat at the top. Now the checkbox centres on one title line (`titleLineHeight`, derived from the type tokens). First attempt used a plain one-line `SizedBox`, which aligned the ring but **cut the real tap area to 18px**, under WCAG 2.5.8 — caught by measuring, fixed with `OverflowBox` so the 48×48 target still paints and still receives taps while only its layout height collapses. Verified: alignment 0.1px, target 48×48, tap fires.

**3. Light-mode surfaces had no separation.** Base and nav overlay measured **1.078:1** — the reported "same color for bg and menu". The light base moved from L=0.974 to L=0.945 (#EEECEA) so surfaces above it have somewhere to go; overlay-vs-base is now 1.175:1.

**4. The whole light palette was re-solved against that new base — and this is the important part.** Deepening the base invalidated every light-mode contrast value fixed earlier in the session: all 12 custom swatches dropped to ~4.2:1 and three built-in icon colors fell under the floor again. Re-solved in one pass against the final base: 12-swatch ramp, all five built-in `*500` icons, and `slate500` (secondary text, which had dropped to 4.19:1). Everything now ≥4.63:1.

**The gap that let the reported teal bug survive an earlier "all swatches pass" session:** only the 12-swatch CUSTOM palette was being tested. The five BUILT-IN categories render through their own hand-tuned `*500` maps and bypassed that ramp entirely — ochre (the green-teal at hue 153) sat at 4.27:1, periwinkle 4.11:1, neutral 4.50:1, while the suite stayed green. Built-ins now have their own contrast tests.

**A second testing gap, also fixed:** `palette_contrast_test.dart` was asserting against `ColorPrimitives.surface0` while the light theme's real base had moved to `cream0` — measuring against a background the app no longer uses. It now reads both bases from `AmbleTheme` directly, which is the whole point of the file.

**Verification:** token tests 39/39 (8 palette incl. built-ins, 7 elevation incl. new light-separation cases). `zone_container_block_test.dart` 24/24, `task_capsule_text_row_test.dart` 14/14. Golden `timeline_capsule_preview.png` regenerated (3.52% diff — intentional colour change, per the "regenerate rather than tolerate" rule) and inspected: checkboxes visibly sit on each row's first text line. Full sweep 633 passing / 1 failing, that one the known pre-existing `place_task_line` case.

## 2026-09-12 — Light-mode bg/menu: the real cause was the scaffold painting the wrong token

Reported again after the previous attempt: "3 cant see resolved."

**The previous fix was correct and invisible.** Deepening `cream0` changed a token nothing painted. `main.dart` wired `scaffoldBackgroundColor` to `colorSurfacePrimary` — a raised PANE, pure white in light mode — instead of `colorSurfaceBase`, the ground. Measured: the light scaffold and the white floating nav were **both #FFFFFF, contrast ratio 1.000, byte-identical**. That is the reported "bg and menu same color", and no amount of base-token tuning could have shown up while the page painted the pane.

**Two fixes, both structural:**
- `scaffoldBackgroundColor` now paints `colorSurfaceBase` (level 0).
- Dark mode's level 0/1 pair was **inverted** — `ink900` (the deepest step, and the directly-specified #121110 body color) was the raised pane while the lighter `ink800` was the ground. Un-inverted. This only stayed hidden because the scaffold painted primary, so the base was never on screen.
- Light `colorSurfacePrimary` moved from pure white to `cream1`; white is reserved for the floating overlay, which otherwise had nowhere above the pane to go.

**Both ramps now step in one direction, measured:** light `#EEECEA → #F6F4F2 → #FFFFFF` (1.175:1 base→overlay); dark `#121110 → #171615 → #272625` (1.252:1). Text re-verified on the new bases: light primary 15.01:1, secondary 4.66:1, accent 4.54:1; dark primary 15.83:1, secondary 7.61:1.

**The lesson, encoded as tests.** The token values were right for two sessions running while the screen was wrong, because every test asserted on tokens and none asserted on what the app actually paints. Added three cases: one reading `main.dart` to confirm the scaffold wires to `colorSurfaceBase`, one asserting base and overlay are never equal in either theme, and one asserting each theme steps base → pane → overlay strictly in one direction. The first is confirmed to fail when the wiring is reverted.

**Verification:** elevation tests 10/10, token suite green. Golden `timeline_capsule_preview.png` regenerated for the intentional surface change and inspected. Full sweep 635 passing / 2 failing — both the known pre-existing pair (`place_task_line`, and the `multi_task_group_resize` case that passes in isolation).

## 2026-09-12 — Light-mode surface/card separation, badge tint saturation

**1. Manage-screen row cards were indistinguishable from the page in light mode.** `colorSurfaceSecondary` (a nested row/card) equalled `colorSurfacePrimary` exactly — both `cream1` — so a Manage list row painted the same value as the pane behind it. Dark mode's equivalent (`ink700` vs `ink800`) is genuinely distinct, which is why the same screen looked correct there and broken in light. Rebuilt the light ramp as a real 4-step climb: `cream0` (base) → `cream1` (pane) → `cream2` (card/row, NEW) → white (overlay). `colorSurfaceSecondary` now points to `cream2`.

**2. `colorSurfaceTimeline` was a leftover pure-white value from before the base moved off true white.** Measured 1.175:1 brighter than the surrounding cream page — a visible stray-white patch behind the whole Timeline canvas (all three view modes), most noticeable in Zone view where card fills sit close in tone to both surfaces. Now `cream1`, matching dark mode's own `colorSurfaceTimeline == colorSurfacePrimary` equivalence.

**3. Category badge tints strengthened.** Reported directly, after contrast math had already passed: "color of container is pale on light mode, needs to be stronger" — specifically the personal (blue) and work (teal) badges. This was a saturation/vividness complaint, not a contrast failure (confirmed with the user via AskUserQuestion): icon-on-tint contrast measured 4.5-4.9:1 even at the old near-white tint lightness (L≈0.95-0.98). Deepened all four category tints to L=0.90 with chroma raised ~1.45x (clay/ochre/periwinkle/berry — neutral stays achromatic by design). Icon-on-badge contrast drops to ~4.0-4.2:1, still clear of the 3:1 floor a decorative icon glyph needs.

**Verification:** 2 new test groups in `palette_contrast_test.dart` (tint-vs-white deepening check, icon-on-tint 3:1 floor) — confirmed to fail against the old pale tint values when reverted. Full sweep 637 passing / 3 failing: the 2 known pre-existing cases plus the golden `timeline_capsule_preview.png`, regenerated for the intentional color change and inspected (page background now visibly cream, badge fills visibly more saturated). `flutter analyze` clean apart from the 2 pre-existing `modalTitle` warnings.

**Open — awaiting user screenshot:** reported checkbox misalignment on Task (spatial) view when zones exist, and an oversized right-edge gap. Measured: `TaskCapsuleTextRow`'s own checkbox IS already flush against its container's right edge (0px gap) — the container itself stops 48px from the true screen edge when zones exist (24px `rightEdgeInset` + 24px reserved for the rotated zone-name label) vs 24px with no zones. Two fix directions identified (shrink the zone-label lane vs. let the checkbox reach the true edge with the label sharing its outer tap-target margin) — user is sending a reference screenshot before choosing.

## 2026-09-12 — Task-view checkbox pushed to the true edge; checkbox ring 18px

**Resolved the open item from the previous entry.** User sent a reference screenshot marking the intended checkbox position with a red line, directly at the true screen edge — confirming "option 2" from the earlier question: the checkbox reaches the true edge; the rotated zone-name label shares that same outer margin rather than the checkbox column being pushed inward to avoid it.

**Fix.** `textColumnRight` (the right-side inset for a Task-view row's time/title/checkbox column) used to be `rightEdgeInset + (zones.isEmpty ? 0 : zoneLabelGutterWidth)` — 48px whenever any zone existed on the day, vs 24px otherwise. All 4 call sites (plain task row ×2, pending-draft pill, overlap-cluster row) now use `rightEdgeInset` alone, unconditionally. `ZoneNameLabel` already renders via `IgnorePointer`, so it can't steal the checkbox's taps even where the two now visually share space.

**Dead code removed, not left behind.** `_zoneLabelGutterWidth` had zero remaining callers once all 4 sites were fixed — deleted per the project's no-dead-code rule, along with the stale doc comment on `textColumnRight` that still described the old widened behavior.

**Checkbox ring: 24px → 18px**, per direct instruction. Implemented as a plain literal in `CompletionCheckbox` rather than reusing `theme.spacingLg` (still 24px, still used elsewhere for unrelated spacing) — a checkbox's own visual size is a distinct decision from the generic spacing scale. The 48px tap target (WCAG 2.5.8) is untouched; only the visible ring and its checkmark icon scale down.

**Verification:** `completion_checkbox_size_test.dart` (3 cases: ring is 18px not 24px, tap target still 48px, checkmark icon scales proportionally) and `task_row_right_edge_test.dart` (2 cases: the dead helper is actually gone, every right-inset call site is the plain unconditional form) — all 5 confirmed to fail against the pre-fix code when reverted. Full sweep 637 passing / 3 failing: the 2 known pre-existing timeline cases plus the golden, regenerated for the checkbox size change and inspected (rings visibly smaller). One additional tracked_behavior failure seen in the full sweep was confirmed to be a parallel-run flake — passes 5/5 in isolation, unrelated to any file touched this session. `flutter analyze` clean apart from the 2 pre-existing `modalTitle` warnings.

## 2026-09-12 — Zone-view task row alignment with time hidden; Task-view checkbox column narrowed further

**1. Zone card task badges misaligned with the zone name (time hidden).** Reported directly, clarified via AskUserQuestion: "inside zone items have some additional padding/margin left than name of zone" meant specifically — with time visibility off, a task row's category badge (and title after it) should start at the same x as the zone name above it, not sit offset from it. Measured: with `timeRangeVisible`/`durationVisible` both false, `timeLabel` becomes `''`, but the empty `Text` inside `IntrinsicWidth` still reserved ~23px (an empty string doesn't collapse a text box to zero — line-height/padding remain), plus a trailing `spacingSm` gap — the badge sat at x=31 against the zone name's x=16.

**Drag tradeoff, resolved via AskUserQuestion.** The time column's `GestureDetector` also carried the row's drag-to-reschedule gesture. Collapsing it to zero width when empty (`if (timeLabel.isNotEmpty) ...`) removes that gesture's home. Confirmed directly: "drag should be pill only" — the category badge now carries the drag handlers instead, but ONLY when the time column is gone (with time visible, the time column keeps the job unchanged, verified with its own regression test). One edge case handled: an uncategorised task (`category == null`) has no badge either, so with both time AND category absent there would be no drag surface at all — added a narrow fallback wiring the whole-row `GestureDetector` for drag in that specific combination only, not broadly (avoiding the exact "drag fights vertical scroll" bug the narrow-column design originally existed to prevent).

**2. Task-view checkbox column narrowed beyond the standard page margin.** Follow-up to the prior session's "checkbox to the true edge" fix: even at `rightEdgeInset` (24px, the same margin every screen in the app uses), there was still "room to the right," confirmed directly as specific to this column, not a request to shrink the shared page margin. New `textColumnRightInset` (`spacingMd`, 16px) — applied to all 3 `textColumnRight` call sites (plain task row, pending-draft pill) and the overlap-cluster row's own `right:`. `rightEdgeInset` itself is untouched and still governs pill positioning and the drag-lift frosted pane, which were never part of either report.

**Verification:** `zone_container_block_test.dart` gained 3 cases (badge aligns with zone name when time hidden, stays unchanged when time IS visible, drag still fires via the badge) — the alignment case confirmed to fail when reverted. `task_row_right_edge_test.dart` rewritten for the new narrower inset, with a dedicated case confirming pill/drag-lift geometry stayed on the wider `rightEdgeInset` — all 4 confirmed to fail against the pre-fix composition. Full sweep 642 passing / 1 failing, the known pre-existing `place_task_line` case. `flutter analyze` clean apart from the 2 pre-existing `modalTitle` warnings.

## 2026-09-12 — Zone names subtler + new colorTextTertiary token; task/zone-name fonts shifted one scale down

**1. New `colorTextTertiary` semantic token.** Requested directly: "Make zone names even subtler color (add new subtler semantic token)." The rotated Task-view zone-name label and the Zone-view card header both used `colorTextSecondary` (4.66:1 light / 7.61:1 dark) — the same color as ordinary secondary text everywhere else, so darkening/lightening it globally would have affected unrelated UI. New token, solved for ~3.3-3.4:1 against each theme's own base — past WCAG's 3:1 floor for large/decorative text, clearly lighter than secondary without fading to illegible. Backed by two new primitives: `slate400` (light, 0.600 lightness, between the existing `slate500`/`slate300`) and `sand350` (dark, 0.520 lightness, between `sand400` and the ink ramp). Wired into both zone-name render sites — the rotated label (`ZoneNameLabel`) and the zone card's own header title, whose paired time-range text moves to the same subtler value (preserving an earlier confirmed "these two match" decision, just at the new subtlety).

**2. Task/zone-name fonts shifted one step down the type scale, across all 3 task-size settings.** Requested directly: "zone names and task names (across 3 different size settings) reduce 1 scale down." The active fonts were sm=12/md=14/lg=14 (lg deliberately reused md's font per an earlier confirmed "large pill, same font as medium" decision — only its badge grew). New `TypePrimitives.size0` (11px) added — nothing existed below the previous smallest (`size1`, 12px). `textTaskTitleSm/Md/Lg` moved to `size0/size1/size2` (11/12/14).

**lg's pairing reversed, confirmed via AskUserQuestion.** Reducing lg "one step down" had two readings: follow md down to the same value (preserving the "same as medium" pairing), or take its own genuine one-step reduction from its own unused 16px baseline. Confirmed the latter — `_resolveTaskSize` (main.dart) now assigns `textTaskTitleLg` directly instead of borrowing `textTaskTitleMd`, so lg (14px) is once again strictly larger than md (12px), reversing the 2026-09-10 "same as medium" decision specifically for this session's request. Badge diameters are UNCHANGED — the request was specifically about text.

`textTaskTitle` (the shared, ACTIVE task-title token every task-related label across Task/List/Zone view reads — confirmed by its own doc comment as already covering "Zone view's in-container rows and its own zone header") is what makes "task names and zone names" a single fix rather than two: the Zone-view card header already read this same token before this session, so shrinking it there was automatic once the underlying rungs moved.

**Verification:** `task_size_scale_test.dart` rewritten for the new font values and the reversed lg pairing (11 cases, all passing, including a case that specifically checks lg's font is once again strictly greater than md's rather than equal). `color_text_tertiary_test.dart` (new, 6 cases: distinct value in both themes, genuinely subtler than secondary, clears the 3:1 floor, survives copyWith/lerp). `zone_background_block_test.dart` and `zone_container_block_test.dart` each gained a color-assertion case, both confirmed to fail when reverted to `colorTextSecondary`. Golden `timeline_capsule_preview.png` regenerated for the font-size change and inspected.

**A real detour, logged for the record:** the full sweep initially showed 14 failures instead of the expected 1-2, including all of `multi_task_selection_test.dart` and the `multi_task_group_move/resize` files. Investigated at length (surgical reverts of the font/badge changes in isolation, a git-stash comparison that turned out unreliable against this repo's deeply layered uncommitted state) before finding the real cause in the test's own hit-test failure output: `Offset(243.0, -40.5)` — the fixed `hour: 9` task scheduled time was rendering off the TOP of the viewport at the actual wall-clock run time (14:44). This is a **pre-existing, already-documented time-of-day-dependent test fixture bug** (see this same log's own 2026-09-07-era entry: "confirmed to fail with the SAME pre-existing hardcoded-hour bug... at the current ~22:30 wall-clock time"), unrelated to anything in this session. Excluded these 3 known-flaky files from the sweep per the established convention; the remaining 640 pass, 1 known pre-existing failure (`place_task_line`) plus the expected golden regen.

`flutter analyze` clean apart from the 2 pre-existing `modalTitle` warnings.

## 2026-09-12 — Manage screen simplified to Tasks-only, cards removed; Zone view text/badge restyled; new duration badge

**1. Manage's four sub-tabs (Tasks/Templates/Zones/Categories) removed entirely — kept as Tasks only.** Requested directly: "remove tabs tasks templates zones categories... only keep tasks." Checked first whether this would strand Templates/Zones/Categories with no UI entry point at all — confirmed they're independently reachable via Settings (`showTemplateListScreen`/`showZoneListScreen`/`showCategoryListScreen`), so nothing became a dead end; Manage simply stops being a second path to them. `InboxScreen` converted from `ConsumerStatefulWidget` (needed local `_tab` state) to a plain `ConsumerWidget`. `InboxTabSwitcher`/`InboxManageTab` deleted outright (zero remaining consumers). `inbox_manage_tabs_test.dart` (entirely about the removed tabs) deleted; `inbox_card_alignment_test.dart`'s own tab-alignment case removed, its remaining background/fade tests kept (unaffected by the tab removal).

**2. Manage task cards removed.** Requested directly: "remove cards from Manage." The row's `Container` (fill + shadow + rounded corners + full padding) is gone — a plain `Row` inside a `Padding` (vertical only, for tap-target height), separated from neighbours by the list's own separator gap. `inbox_card_alignment_test.dart`'s shadow-assertion test reversed into a no-decoration assertion, confirmed to fail when the card decoration is restored.

**3. New duration badge on Manage task rows.** Requested directly: "duration in tasks move to the right but text size 10px (smallest scale) in a 'badge' surface color next to current bg." Confirmed via AskUserQuestion that "10px" meant the existing smallest type-scale rung (`textTaskTitleSm`, 11px — added earlier this session), not a new literal primitive. Renders only when `Task.durationMinutes` is set (Inbox tasks are unscheduled and the field is genuinely nullable) — `colorSurfaceSecondary` fill (one step up from the screen's own `colorSurfaceTimeline` background, per this session's earlier elevation-ramp work), `radiusSm` corners, sitting between the title and the trailing "+" button.

**4. Zone-view task/zone-name font: one scale up.** Requested directly: "Size of text in zone view (task name one scale up)." Reverses an explicitly-documented 2026-09-10 decision that Zone view tracks the shared `textTaskTitle` token "with no relative step." New `AmbleTheme.textTaskTitleZone` field, resolved in `main.dart`'s `_resolveTaskSize` as always one rung above whichever `textTaskTitleSm/Md/Lg` the active Task-size setting selects for `textTaskTitle` (sm→md's size, md→lg's size, lg has no larger rung so stays at lg's own size — there is nowhere further up to go). Every `textTaskTitle` use inside `zone_container_block.dart` (header, task row's time+title, external-event row's time+title) moved to this new token together, since the task row and the external-event row are built to visually line up in the merged list — bumping only one would have broken that alignment.

**5. Zone-view category badge: smaller, and a rounded square instead of a circle.** Requested directly: "the emoji and circle (make actually same shape as timeline, not circle but rounded square) smaller." Reverses ANOTHER earlier session's decision (enlarging this badge to a fixed `sizeTaskBadgeXl` circle, confirmed at the time as "tasks pill... circle with icon large"). Now `theme.sizeTaskBadge` (the SAME active, resolved size `TaskCapsuleBlock`'s own pill rail already uses) with `radiusSm`-rounded corners (matching that same pill's own shape exactly, including its own documented reasoning for avoiding a fully-round `radiusTaskPill`) — so the Zone-view badge now matches the Task-view pill in both size and shape rather than being its own separately-scaled circle.

**Verification:** `inbox_duration_badge_test.dart` (new, 6 cases: shows/hides correctly based on nullable duration, correct hours-and-minutes formatting, smallest-scale font, correct surface color, correct position) — key assertions confirmed to fail when the badge is removed. `zone_container_block_test.dart` gained 3 cases (one-scale-up font, badge size, badge shape) — all 3 confirmed to fail against the pre-change code. `inbox_card_alignment_test.dart` updated (5/5, tab-alignment case removed, card-decoration case reversed and confirmed to fail when the card returns). Full sweep 640 passing / 1 failing, the known pre-existing `place_task_line` case (same 3 time-of-day-flaky files from earlier this session excluded per that established convention). Golden `timeline_capsule_preview.png` unaffected — confirmed it still passes as-is, since it captures Task view's own capsule, not Zone view. `flutter analyze` clean apart from the 2 pre-existing `modalTitle` warnings.

## 2026-09-12 — Nav redesign: Task view split out as its own tab, new shared top calendar header replaces the old bottom day-strip, List view removed

Requested directly from an annotated reference screenshot, scoped through 11 rounds of clarifying questions before building (see docs/DECISIONS.md's matching entry for the full confirmed-scope list — nav restructure, header placement, bottom-bar replacement, List-view removal, sync placeholder, today-button behavior, icon-row mapping, pen-icon Edit control, one-pass build, fixed week-grid, and month-tap-steps-by-week).

**Shipped:** `TimelineScreen` gained a required `TimelineDisplayMode { spatial, zone }` param — Task view (spatial) and Timeline (zone) are now two separate permanent bottom-nav tabs instead of one screen with an in-screen switcher. `main.dart`'s `AmbleHome` reordered to 5 tabs (Task view, Timeline, Manage, Tracked, Settings) with hidden labels, default tab changed from Inbox (index 1) to Task view (index 0) — all 4 separate hardcoded "index 1 = home" assumptions in `main.dart` found and fixed. New `lib/features/timeline/app_calendar_header.dart` (month stepper, today-jump button, sync placeholder icon, Edit Mode pen-icon toggle, fixed S–S week grid) shared by both tabs, replacing `DayStrip`'s day-navigation half entirely; `DayStrip` deleted outright (zero remaining consumers), its "+"-button half survives as a bare `AppBottomExtensionBar`. List view (the third, dev-flag-gated Timeline mode) dropped from user reach entirely — dead code deleted rather than left stale (`day_strip.dart`, `list_mode_clustering_test.dart`, `list_mode_task_filters_test.dart`, `day_strip_view_cycle_test.dart`, `day_strip_stability_test.dart`, one case in `task_event_lane_sharing_test.dart`).

**Bug found and fixed before shipping:** `_MonthStepper`'s month-name text used `theme.textHeadline` (32px) inside a `Row(mainAxisSize: MainAxisSize.min)` in a ~240px-wide constrained slot — overflowed by 80px, cascading into ~51 failing Task-view tests. Fixed with `theme.textTitle` (20px) + `Flexible`/ellipsis, no more `mainAxisSize: min`.

**Verification:** `flutter analyze` clean (2 pre-existing unrelated warnings only). `test/features/timeline/` full sweep: 349 tests, 13 failing — all 13 confined to `multi_task_group_move_test.dart`/`multi_task_group_resize_test.dart`, both already-documented (ERROR_LOG.md) wall-clock-time-dependent fixture flakes (hardcoded `hour: 9/12/13` vs. real "center on now" scroll behavior), reproducible before this session's changes too. Every test that failed due to the overflow bug now passes.

**Open/not done this session:** no dedicated `AppCalendarHeader` unit/widget tests yet (week grid, today-button, sync no-op, pen-icon toggle, month stepper tap/swipe) — flagged, not silently skipped. No visual/screenshot verification against the reference image yet — only structural/logic verification so far. CONSTITUTION.md's Edit Mode section was annotated (not rewritten) to record the pen-icon reversal of its locked "styled as a link, not an icon" principle — see docs/DECISIONS.md.

## 2026-09-12 (follow-up) — Edit Mode close control fixed; nav bar restyled as a fully-rounded floating pill; "+" separated into its own floating button

Two more reports off the same reference screenshot, after visually reviewing the shipped redesign above.

**1. Edit Mode had no visible way to exit.** `AppCalendarHeader` (including its own Edit Mode toggle) was hidden ENTIRELY while Edit Mode was active — a real gap, reported directly: "we need close in the top right corner same style as other icons in top right corner (edit that initiated it)." `AppCalendarHeader` now stays mounted throughout Edit Mode, collapsing to just that one control — the SAME toggle, now showing `Icons.close_rounded` instead of the pen while active — right-aligned in the same top-right spot, same subtle-outline style as every other header icon. `timeline_screen.dart`'s own gating `if` no longer hides the header for Edit Mode, only for a live quick-create draft (which has no control on the header that needs to stay reachable).

**2. Nav bar restyled to match a reference screenshot exactly: "match aesthetics and nav is just 5 items + its outside."** Two changes: (a) the bottom nav is now a fully-rounded floating pill, margined on every side (was: bottom-flush, bottom-corners-only, welded to a bar above it) with a circular selected-tab indicator filled with `colorSurfaceSecondary` — confirmed directly as a surface color, not the accent ("match and not accent color like ours now but more surface (next in order)"); (b) the "+" create button is no longer part of any bar at all — it's now `AppFloatingCreateButton`, an independently positioned floating circle, matching the reference's "+" sitting outside the pill. `AppBottomExtensionBar` (the widget that used to weld the "+"/day-strip/tab-switcher to the nav as one connected pane) is deleted outright — Timeline, Manage, and Tracked each now use `AppFloatingCreateButton` instead. Tracked's own weekly/monthly/six-monthly view-cycle switcher (previously living in that bar's `leading` slot) moved to a new top-right header row on that screen — confirmed directly: "move to the top right functional icons like we do on timeline" — using a promoted, now-shared `AppSubtleIconButton` (lifted out of `AppCalendarHeader`, which used it privately first).

**Also fixed as a consequence:** `quick_create_overlay.dart`'s `_backdropHeight` used to add `DayStrip`/`AppBottomExtensionBar`'s own height to the space it covers while a draft is live; with that bar gone, it now only needs to reach `NavigationBar`'s own height plus the bottom safe-area inset.

**Verification:** `flutter analyze` clean (2 pre-existing unrelated warnings only). New tests: `app_subtle_icon_button_test.dart` (5 cases), `app_floating_create_button_test.dart` (3 cases), `floating_nav_pill_test.dart` (3 cases, pill corners/margin/indicator-color) — all passing. `edit_mode_hides_main_nav_test.dart` rewritten for the new widgets and the header's new collapse-not-vanish behavior (3/3 passing). `test/features/inbox/` + `test/features/tracked_behavior/` full sweep: 68/68 passing. `test/features/timeline/` full sweep: 356 tests, 14 failing — all 14 confined to the same already-documented (ERROR_LOG.md) wall-clock-time-dependent fixture family as before (`multi_task_group_move_test.dart`, `multi_task_group_resize_test.dart`, `multi_task_group_delete_test.dart`), not caused by this change. Old `connected_bottom_pane_test.dart` (tested the now-removed welded-pane relationship) deleted rather than left stale.

**Not done:** visual/screenshot verification against the reference image still hasn't happened — only structural/logic verification (tests, analyze) so far.

## 2026-09-12 (second follow-up) — nav pill made fully round + shorter, selected-icon size and indicator direction fixed; top scroll-fade added to Zone view

A visual review of the shipped pill turned up 4 more concrete corrections, plus one gap only visible once actually compared against the reference image side by side.

**1. "Full round, not just round corners."** `borderRadius: BorderRadius.circular(theme.radiusXl)` only rounded the corners short of a true stadium shape. Changed to `BorderRadius.circular(999)` — a value the engine clamps to true full-round ends regardless of the pane's own height, the same trick as CSS's `border-radius: 9999px`.

**2. "Too big height."** Material's own `NavigationBar` default is 80px. Added an explicit `height:` (`spacingXl * 1.4` ≈ 56px).

**3. "Selected item too small."** Both selected and unselected icons rendered at Material's flat default (24px) — no distinction at all. A local `Theme`-scoped `NavigationBarThemeData.iconTheme` (a `WidgetStateProperty`) now sizes the selected icon larger than the unselected one, without touching the app's own global `ThemeData`.

**4. "Wrong surface (selected has darker surface, should have lighter) and surface jump too big."** The prior fix used `colorSurfaceSecondary` for the indicator, reasoned as "one step up the elevation ramp" — but in dark mode `colorSurfaceSecondary` (`ink700`) is actually DARKER than the pane's own `colorSurfaceOverlay` fill (`ink650`, already the ramp's lightest step), an inversion missed because the two tokens' relative ordering wasn't checked against the actual primitive values before picking one. Switched to `colorSurfaceField` — confirmed lighter than `colorSurfaceOverlay` in dark mode and a single, correctly-sized step in light mode (not the double indirection the "2 jumps" complaint was pointing at).

**5. Zone view had no top scroll-fade at all** — reported directly, pointing at the same reference image: "hard edge here... should be gradient that fades under it." Task view's own `_DayTimeline` already had one (from an earlier session, fading its own top edge against whatever sits above it — now `AppCalendarHeader`); `ZoneDayTimeline` never had the equivalent. Added the same `AppTopScrollFade` in the same position, wrapping its previously-bare `TweenAnimationBuilder`/`ListView` in a `Stack`. (A first attempt added a THIRD fade at the outer `AnimatedSwitcher` level, which would have doubled up with `_DayTimeline`'s existing one on Task view — caught before shipping by checking `_DayTimeline`'s own code first, and reverted in favor of this narrower, non-duplicating fix.)

**Verification:** `flutter analyze` clean (2 pre-existing warnings; also fixed one previously-hidden `curly_braces_in_flow_control_structures` info that a `dart format` pass over an unrelated 2026-09-12-mechanical-edit file exposed). `floating_nav_pill_test.dart` expanded to 7 cases (added: full-round-stadium check, height-under-80 check, selected-icon-larger check, dark-mode indicator-lighter-than-pane check) — all passing, old indicator-color assertion updated for the token change. `zone_day_timeline_list_test.dart` gained a fade-presence case (18/18 passing). Full `test/features/timeline/` sweep: 357 tests, 14 failing, all still confined to the same pre-existing wall-clock-time-dependent fixture family (`multi_task_group_move/resize/delete_test.dart`) — unrelated to this change.

## 2026-09-12 (third follow-up) — nav icons bumped further, unwanted background patch around the floating pill removed

Two more corrections from a further visual review, one pointed at with an annotated screenshot arrow.

**1. Nav icons bumped again.** Reported a second time as still too small, alongside "perhaps padding was too big": `spacingLg`/`spacingMd` (24/16px) barely read as different from Material's own flat 24px default. Bumped to `spacingXl`/`spacingLg` (40/24px) — a visibly bigger jump — and `_navBarHeight` correspondingly bumped from `spacingXl * 1.4` to `* 1.6` (~64px, still well under Material's 80px default) so the larger icons/indicator have room without feeling cramped.

**2. Unwanted background patch around the floating nav pill, hex `#121110`.** `Scaffold.scaffoldBackgroundColor` was wired to `colorSurfaceBase` (`ink900` in dark mode, `#121110`) — a deliberate earlier fix for a genuinely different bug (light mode: the scaffold and the pure-white floating nav were byte-identical). But now that the nav pill floats with margin on every side rather than sitting flush to the screen edge, that margin gap exposes `scaffoldBackgroundColor` directly — and every real screen (Timeline, Inbox, Tracked, Settings) already paints its own full-bleed `colorSurfaceTimeline` (`ink800`), a visibly LIGHTER value. The mismatch read as an unwanted extra dark patch around the pill — reported directly, with an annotated screenshot arrow pointing at it and the literal hex. Re-wired to `colorSurfaceTimeline` instead; confirmed this doesn't reintroduce the original bug (`colorSurfaceTimeline` is still distinct from `colorSurfaceOverlay`/the white nav pill in light mode).

**Verification:** `flutter analyze` clean. `floating_nav_pill_test.dart`'s existing icon-size test (a relative comparison) still passes unchanged. `elevation_direction_test.dart`'s source-string assertion pinning the OLD `scaffoldBackgroundColor` wiring updated to check the new one, plus a new case confirming `colorSurfaceTimeline`/`colorSurfaceOverlay` stay distinct in both themes (the original bug this whole test group guards against) — 11/11 passing. `test/core/tokens/` full sweep: 46/46 passing.

## 2026-09-12 (fourth follow-up) — reverted the selected-icon size bump: it stretched the circular indicator into an oval

A screenshot caught the actual on-screen effect of the earlier "selected item too small" fix: growing only the selected icon made Material's own circular indicator stretch wider to contain it, turning the clean circle into a pill/oval — never what was asked for. Reported directly: "active icon should not enlarge... ask was to change color, not [size/shape]" — confirming the ORIGINAL ask (several sessions back) was purely a recolor, and the icon-size growth was a wrong-direction fix layered on top of it by mistake.

**Reverted:** both nav-bar icon states now render at the same size (Material's own flat default, `spacingLg`/24px) via `WidgetStateProperty.all` instead of the state-branching `resolveWith` from the prior two fixes. `_navBarHeight` correspondingly walked back from `spacingXl * 1.6` to the original `* 1.4` (~56px), since the taller ratio was only ever there to give the now-reverted larger icon room.

**Kept:** the indicator's own `colorSurfaceField` fill (the actual, correct color fix from two sessions back) is untouched — this was never about the color, only the size/shape that had been mistakenly bundled in alongside it.

**Verification:** `flutter analyze` clean. `floating_nav_pill_test.dart`'s icon-size test rewritten from "selected is larger" to "both states are the same size" (the correct, now-restored invariant) — 7/7 passing (a genuinely-oval-detecting test was attempted via `NavigationIndicator`'s own render size, but that widget's box is a fixed 64×32 regardless of shape — Material implements the circle by clipping/inset within that box, not by resizing it — so that assertion couldn't distinguish circle from oval and was dropped in favor of the icon-size-equality check, which does encode the actual root cause).

## 2026-09-12 (fifth follow-up) — Manage row's category badge and title were bypassing the shared "Task size" setting entirely

Reported directly: "manage items should have same size as zone view." Traced to `_InboxListItem` (Manage/Inbox screen) using hardcoded `theme.spacingXl` (40px) for its category badge and `theme.textBody` (16px) for its title — both completely independent of the app's one shared "Task size" setting (sm/md/lg), which every other task row (`TaskCapsuleBlock`, used by both Task view and Zone view) already resolves through via `theme.sizeTaskBadge`/`theme.textTaskTitle`. Manage rows were rendering visibly larger than either of the other two views, and would stay that size regardless of what the user picked in Settings.

**Fix:** both values switched to the same resolved tokens (`theme.sizeTaskBadge`, `theme.textTaskTitle`) `TaskCapsuleBlock` already uses — no new tokens, no new setting, just routing this one screen's row through the mechanism that already existed everywhere else. The duration badge's own `textTaskTitleSm` font (added in an earlier session) was already correct and untouched.

**Verification:** `flutter analyze` clean. `inbox_duration_badge_test.dart` gained a case pinning both the title's font size and the category badge's circle dimensions to `theme.textTaskTitle`/`theme.sizeTaskBadge` directly (confirmed to fail against the pre-fix hardcoded values) — 7/7 passing. Full `test/features/inbox/` sweep: 34/34 passing.

## 2026-09-12 (sixth follow-up) — Edit Mode resize-drag edge labels and armed-task tap behavior corrected

Three reports off real Edit Mode usage, none from a reference screenshot this time.

**1. Bottom-resize's end-time label didn't move on Y.** `_liveEdgeTimeLabels`'s `_isResizing` branch positioned the live END label at the pill's stale RESTING height (`_pillHeight` off `widget.task.durationMinutes`, the pre-drag value), not the live preview height — so the label sat frozen at its starting position instead of tracking the finger, unlike the top-resize case (which happened to work already, since the whole block's own `top` shifts during a top-resize and the label rides along with it for free). Fixed by computing the SAME live pixel height `TaskCapsuleBlock` itself is being resized to (`_previewDurationMinutes * pixelsPerMinute`, floored at the pill's own minimum width) and positioning the label there instead.

**2. Both edge labels now show during EITHER resize, not just the moving edge.** Reported directly: the top-resize used to hide the bottom label and vice versa. The original design reasoned the anchored edge was "already shown by the resting label" — but the resting labels (`_edgeTimeLabels`) and the live-drag labels (`_liveEdgeTimeLabels`) are mutually exclusive in the caller's own `if`/`else if`, so a resize was leaving the OTHER edge with NO time label at all, not merely deferring to an existing one. Both branches (`_isResizing`/`_isResizingTop`) now return both labels. The top-resize case also needed the SAME live-height fix as #1 for its own now-newly-shown end label, for the identical reason.

**3. Tapping an armed (long-press-wiggling) task a second time now closes its arm instead of opening the detail sheet.** Reported directly: "tapping on it again it's not opening detail, but stopping edit mode (wiggling)." `_effectiveOnTap` previously always cleared the arm AND opened the sheet on any tap, per an earlier work order ("keep single tap to open edit sheet") that never actually covered the case of tapping the SAME task twice. Tapping a DIFFERENT task while one is armed is unchanged — that still opens the tapped task's own sheet and clears the old arm.

**4. Tapping empty timeline space while a task is armed now ONLY closes the arm — it no longer also starts a quick-create draft.** Reported directly: "its not triggering (task creation if tapped on timeline) but stopping wiggling, so actually... tap on timeline anywhere stops wiggling." `onEmptyTap` used to unconditionally clear the arm AND start a draft on the same tap; now it checks whether a task WAS armed before clearing, and returns early (skipping draft creation) if so. Tapping empty space with nothing armed is unchanged.

**Verification:** `flutter analyze` clean. `drag_move_time_label_test.dart`'s three resize-label tests rewritten from "exactly one label, the moving edge" to "both labels present, correct times" — 7/7 passing. `armed_edit_task_test.dart`'s "single tap on armed task" test reversed to match the new close-not-open behavior, plus two new cases (tap-empty-while-armed skips draft creation; tap-empty-with-nothing-armed still creates one as before) — 8/8 passing. `task_edit_time_labels_test.dart`'s one test that depended on the old tap-opens-sheet behavior updated — 3/3 passing.

## 2026-09-12 (seventh follow-up) — resize drags now track the finger; bigger resize targets; edit mode exits on navigation

Four reports from real Edit Mode use, plus one latent test bug they exposed.

**1. Resize lagged behind the finger, on both edges — for two different reasons.** Reported directly: "when resizing the size of pill should follow (resize top does it) but resize bottom is animated so pill catches up... and also when resize top seem that pill is moving not only resizing top... and it moves and 'settles' with animation." Two separate animations, one per edge: the BOTTOM edge lagged because `TaskCapsuleBlock` sizes its pill with an `AnimatedContainer` at `motionSlow` (there so a duration change from the edit modal or a cascade visibly grows the pill — right for those, wrong under a finger); the TOP edge lagged because the block's own `AnimatedPositioned` only zeroed its duration for `_isDragging`, and a top-resize moves `top` too. Fixed both: a new `TaskCapsuleBlock.isResizing` zeroes the height animation for the length of the gesture only, and `_isResizingTop` joins `_isDragging` in the position animation's zero-duration condition. Non-gesture repositioning still eases exactly as before.

**2. Resize handles were too small, and "outward" turned out to be impossible.** Requested directly: "handle hit/active area should be larger outward (but not inward, bear in mind smallest pill size has also drag as move the entire pill interaction so we need affordance for all resize and move 3 different interactive hotspots)... and sides also difficult to catch." Built the outward version first — and probed it before trusting it: a throwaway widget test showed a gesture box positioned past its parent's bounds **paints but never receives a tap**, vertically or horizontally, `Clip.none` notwithstanding. The whole approach was unshippable, caught before shipping. See docs/ERROR_LOG.md for the finding.

Redesigned within the real constraint: handles now grow INWARD from each edge while their visible bar is pinned to that edge (`ResizeHandle.barAlignment`), so the target genuinely gets bigger and the grab affordance stays where the user aims. New `taskResizeHandleHeightFor` raises the target from `spacingMd` (16px) to `spacingLg` (24px) whenever the pill can afford it, and on a pill too short for both, the HANDLES yield rather than the move band — move is the only one of the three hotspots with no alternative entry point (duration is also editable from the detail sheet; position is not). The old `-spacingXs` overhang is gone: it was dead area that merely looked grabbable. `resizeHandleHeightFor` is unchanged and still used by `pending_task_pill.dart`.

**3. Edit mode now exits on navigation.** Requested directly: "changing day, page, or navigating elsewhere basically or activating sheet would exit edit mode." Previously only a tap on the Timeline itself disarmed, so a task kept wiggling behind an open sheet and stayed armed after a day change — its resize handles belonging to a task no longer on screen. Added two `ref.listen`s next to the existing cross-provider reactions (a real `selectedDate` change, and a `pendingTaskDraft` becoming non-null) rather than wiring N navigation call sites, which would have guaranteed the N+1th got missed.

**4. Fixed the long-standing `multi_task_*` flakes rather than re-diagnosing them.** These four files pinned fixtures to literal 9:00/12:00/13:00 while the Timeline scrolls to centre on `now`, so they only passed when the suite ran near those hours — the family already documented several times in ERROR_LOG.md. This session's earlier header/nav work shifted the viewport just enough to pull one more of them over the edge, which is what surfaced it. Bisected against a stashed baseline to prove the failures were pre-existing rather than caused by the resize work (identical counts with and without), confirmed the cause by probing with `now`-anchored fixtures, then anchored all four files properly. `hour` still reads the same at every call site but now means "half-hours apart," which is all these tests ever needed.

**Verification:** `flutter analyze` clean (2 pre-existing unrelated warnings). New `resize_tracks_finger_test.dart` (3 cases) asserts the observable consequence — geometry matches the finger on the next frame with nothing left to settle — and **both animation cases were confirmed to fail against the pre-fix code**, reproducing the exact reported symptoms ("kept growing after the finger stopped", top easing into place). `resize_handle_height_test.dart` gained a 4-case group for the new task-pill sizing, including the three-hotspot contention on the shortest pill. `armed_edit_task_test.dart` gained 3 navigation cases (day change disarms; quick-create disarms; re-selecting the SAME day does not). Timeline sweep went from 369 passing / 14 failing to **382 passing / 1 failing**, the remainder being `place_task_line_test.dart`, verified identical at baseline. Rest of suite 699/1, the remaining one being the documented Saturday-only weekend-anchor flake in `task_providers_test.dart`.

**Not done:** no on-device check of how the larger inward-growing handles actually feel under a thumb — the sizing is reasoned from platform touch-target guidance and the geometry, and verified by test, but "does it feel right to grab" is a judgment only real use can settle.

## 2026-09-12 (eighth follow-up) — resize: the anchored edge really is anchored, and the dragged edge is continuous

The previous pass removed the resize animations but the report came back unchanged: "the blue badge with time responds immediately to the movement, but the pill takes time to animate... when resizing top, I seem to be moving the pill and the bottom part of it is adjusting... and the blue badge with time, end time is moving around while it should remain fixed." Three separate causes, none of which was the animation.

**1. The pill was driven by the SNAPPED duration, so it moved in 5-minute steps.** `durationMinutesOverride` fed `_previewDurationMinutes` (rounded to 5 minutes, ~7px at the default scale) while the time badge changed on those same instants. A text change reads as instant; a 7px geometry jump reads as "catching up." Removing the animation didn't help because the stepping was never an animation — the animation had merely been smoothing it. Live geometry now follows the raw unsnapped offset and snaps only on release, matching the contract move-drag already states for itself; `durationMinutesOverride` widened from `int?` to `double?` so the height is genuinely continuous rather than re-quantised to whole minutes.

**2. A top-edge drag slid the whole pill once it hit the pill-height floor — a real bug.** The drag shifts `top` down by the start delta and shrinks the height by that same delta, so the two cancel and the end stays anchored. But the rendered height is floored at `sizeTaskBadge` (24px ≈ **16 minutes** at the default scale), while the clamp was against `_minDurationMinutes` (**5 minutes**). Between those two floors the height freezes while `top` keeps moving, and the whole block translates downward — exactly "it's almost like I'm moving the pill." New `_minRenderableDurationMinutes` clamps the start delta at the point the pill can no longer shrink, so the cancellation holds for the entire gesture.

**3. The end-time badge inherited that same bug.** It was positioned at the FLOORED live height, so it drifted with the sliding pill. It now sits at the unfloored anchored end, so it stays put while only the start badge moves.

**The tests from the previous pass were measuring the wrong render box.** They asserted on the capsule's root — an `IntrinsicHeight` `Row` whose height comes from the title column, not the duration — so they passed while the pill was still visibly wrong, and the revert-check passed too, rubber-stamping itself. Corrected to measure the rail's own `AnimatedContainer` in both files. Logged in docs/ERROR_LOG.md, along with the `kTouchSlop` trap that made a sub-snap probe measure exactly 0.0.

**Verification:** new `resize_anchored_edge_test.dart` (3 cases: bottom edge anchored through a past-the-floor top drag, end badge fixed while the start badge moves, sub-snap drag moves the pill) plus the corrected `resize_tracks_finger_test.dart`. **Reverting the three fixes fails 4 of the 6 cases** — this time for the right reasons, against the right render box — and all 6 pass restored. `flutter analyze` clean. Timeline sweep 385 passing / 1 failing (`place_task_line_test.dart`, verified identical at baseline: 7+/1- with and without these changes). Rest of suite 699/1, the documented Saturday-only weekend-anchor flake.

**Not done:** still no on-device check of how the resize now feels under a thumb — the continuity and anchoring are verified by measurement, but the tactile judgment needs real use.

## 2026-09-13 — Weekly Zone Authoring Grid: new full-screen destination, zone multiselect, and per-column group move/resize

Work order for a brand-new full-screen destination showing every zone across all 7 days of the week at once, with an explicit stop-and-flag gate on the Zone materialization prerequisite.

**Gate check**: confirmed materialization (real per-occurrence `Zone` instances, shipped 2026-09-09) against the CODE, not just the docs — CONSTITUTION.md's own Zone section still had a stale "no materialization" paragraph at the time, corrected in this session alongside the rest of the work rather than left contradictory. Proceeded on the confirmed-shipped basis.

**Shipped**: `lib/features/zone_grid/` — `ZoneGridScreen` (7 day columns × hour axis, `showZoneGridScreen` entry point wired into `AppCalendarHeader`'s existing utility row), `ZoneGridBlock` (rotated compact-text zone blocks, no color coding), `ZoneGridTab`/`ZoneGridWeek` providers. `ZoneEditSelection` widened from single-select (`String?`) to genuine multiselect (`Set<String>`), mirroring `EditSelection`'s existing shape — the Timeline's own single-zone editing behaviour preserved via a new `selectOnly` method rather than silently becoming multi-select. New `shared/services/zone_group_move.dart` (`computeZoneGroupMoves`) — per-day-column group move/resize with all-or-nothing refusal across the whole selection if any single column can't satisfy zone-to-zone non-overlap, pure logic with 9 passing tests covering that guarantee directly (not just asserted, verified by reverting the fix and confirming the corresponding tests fail for the right reason).

**Judgment calls, all flagged per the work order's own instructions rather than assumed — full reasoning in docs/DECISIONS.md**: group move all-or-nothing (not per-column); the "affect future instances" toggle is group-wide (one switch, not per-zone); the toggle's own ON state has no Zone-level "re-anchor the series" path to call yet (confirmed `ZoneList` has no equivalent of `TaskList.updateTaskWithChangedRecurrence` before assuming one existed) — the toggle renders and tracks state, but flipping it on doesn't yet change anything; the entry point (one icon in `AppCalendarHeader`) was the minimum built rather than new nav structure.

**A real bug found and fixed before shipping**: day-columns after Monday positioned their zone blocks a full 24 hours too low, because `_blockFor` measured every column against the WEEK's shared `rangeStart` instead of that column's own midnight. Found via a widget test's tap landing exactly one `gridHeight` off; full trace in docs/ERROR_LOG.md.

**Verification**: `flutter analyze` clean (2 pre-existing unrelated warnings). `dart format .` clean. New tests: `zone_group_move_test.dart` (9/9), `zone_grid_screen_test.dart` (10/10). Existing-behaviour regression checks: `zone_multi_task_selection_test.dart` (Timeline's own zone selection) 4/4, `app_calendar_header_test.dart` (new icon doesn't break existing assertions) 8/8. Full `test/features/timeline/` sweep: 385 passing / 1 failing (`place_task_line_test.dart`, a pre-existing failure from earlier this session, unrelated). Rest of the suite (`test/shared/`, `test/core/`, `test/features/zones/`, `test/features/zone_grid/`): 575 passing / 1 failing (`task_providers_test.dart`'s recurrence-narrowing case, also pre-existing).

**Not done**: on-device screenshots (requested in the work order) were not captured — verification this session is `flutter analyze`/`flutter test` only. The Events tab is chrome-only, genuinely non-interactive (no `onTap` wired), per scope. The "affect future instances" toggle's ON state and any zone-delete affordance are both out of scope, per the work order's own non-goals and the found gap above.

## 2026-09-13 (same day, follow-up) — On-device screenshot verification found and fixed two more real bugs

Captured the four on-device screenshots the work order required (emulator, `zone_grid_screenshot_main.dart` dev scaffold mirroring `zone_materialization_preview_main.dart`'s pattern): the grid with several zones across different days, a 2-zone multiselect (blue selection outlines on non-adjacent days), the "affect future instances" toggle on a recurring-zone selection, and the FAB opening the existing "New zone" form.

**Bug 1 — scaffold, not the app**: the screenshot scaffold's own single-day test zones (seeded via `ZoneList.createZone(..., anchorDateForRecurrence: day)` with no `recurrenceRule`) rendered identically on all 7 columns instead of one day each — because `ZoneList.createZone` deliberately forces `anchorDate: null` whenever `recurrenceRule == null` (correct, intentional model behaviour: a non-recurring zone is dateless by design). Fixed the scaffold by writing single-day fixtures directly via `zoneRepository.save(Zone.create(...)..anchorDate = day)`, which the `Zone` model itself permits.

**Bug 2 — real app bug**: `_SelectionBar`'s "Affect future instances" row overflowed by 34px at real phone width (Flutter's debug hazard-stripe banner), invisible in `flutter test`'s wider default test surface. Fixed by wrapping the label in `Flexible`/`maxLines: 1`/ellipsis so it shrinks instead of forcing the row wider than available space. Re-verified on-device: clean, no overflow, reads "Affect future ins…" with the switch fully visible. Full trace in docs/ERROR_LOG.md.

**Verification**: `dart format .` clean, `flutter analyze` clean (same 2 pre-existing unrelated warnings), targeted `flutter test` re-run (`zone_group_move_test.dart`, `zone_grid_screen_test.dart`, `zone_multi_task_selection_test.dart`) 23/23 passing. Screenshots reviewed directly (not just captured) — this is what caught both bugs above; automated tests alone had missed both.

## 2026-09-13 (third pass, same day) — Per-instance drag-to-delete, toggle-governs-delete, grid chrome polish

Direct UI feedback on the shipped grid, addressed as a follow-up rather than a new work order: select a single zone occurrence, drag it to a bottom delete target (reusing the Timeline's own `EditModeDeleteTarget` and drag-to-dismiss language, FAB fading to the red remove affordance mid-drag), subtler inter-column dividers with none on the grid's own leftmost edge, zone blocks inset narrower than their lane, and top/bottom scroll-fade gradients (`AppTopScrollFade`, already shared by the Timeline/Inbox/Tracked).

**Shipped**: `ZoneList.deleteZoneSeries` (new, mirrors `TaskList.deleteTaskSeries`) for the toggle-ON "delete this and all future instances" case; `_ZoneGridScreenState`'s drag handlers now check drop position against the delete target BEFORE running the existing group-move/cascade path, matching the Timeline's own delete-short-circuits-cascade discipline; `AppFloatingCreateButton` gained an `opacity`/nullable-`onPressed` pair so it can fade its own icon without breaking its internal `Positioned`'s `Stack`-ancestor requirement.

**Two real bugs caught before shipping, both by the test suite rather than eyeballing**: `EditModeDeleteTarget` was missing its `Positioned(left:0,right:0,bottom:0)` wrapper (rendered unpinned, so every drop silently missed it) — found because a new widget test asserted the delete actually happened, not just that the target was visible. Wrapping `AppFloatingCreateButton` in an external `AnimatedOpacity` crashed immediately with a ParentDataWidget error, caught the same way before ever running on a device.

**Judgment calls confirmed via AskUserQuestion, both flagged rather than assumed**: per-instance delete survives a multi-select (dragging one selected zone deletes only that one) — a deliberate divergence from the Timeline's own confirmed "group delete on drop" behaviour for tasks. The "affect future instances" toggle governs delete-by-drag too, not just move/resize.

**Verification**: `dart format .` clean, `flutter analyze` clean (same 2 pre-existing unrelated warnings). `zone_grid_screen_test.dart` grew 10 → 16 (all passing, including a revert-and-confirm-fails check on the `Positioned`-wrapper fix). `zone_group_move_test.dart`, `zone_multi_task_selection_test.dart`, `app_floating_create_button_test.dart`, `edit_mode_hides_main_nav_test.dart`, `floating_nav_pill_test.dart` all still passing — the `AppFloatingCreateButton` signature change is additive and doesn't disturb its other 3 call sites. Full `zone_grid`/`zones`/`zone_group_move`/`zone_providers` sweep: 44/44. Two test-writing gotchas hit and logged to ERROR_LOG.md: a real async Hive write inside a drag callback needs `runAsync` + a real delay, not a fake-clock `pump`; and `pumpAndSettle` hangs indefinitely once a still-selected zone's `EditModeWiggle` is animating (no fixed-duration alternative existed in this file before — added one).

## 2026-09-13 (fourth pass, same day) — Zone create: time optional (2h default), overlap no longer blocks (squeezes instead), existing-zones browser

Three direct requests on the zone add/edit form: stop requiring start/end to save (default to a 2-hour block starting at the currently-viewed time); stop refusing to save an overlapping zone — squeeze other same-day zones out of the way instead, resizing/pushing them if needed; and list already-created zones as tappable cards under Name on the create flow, mirroring the task-creation flow's own template browser.

**Shipped**: `_ZoneFormScreenState._canSave` no longer requires `_startMinutes`/`_endMinutes` (a half-typed pair — one field filled, the other blank — is still refused, since that's genuinely ambiguous, not "intentionally blank"). `_resolvedStartMinutes()`/`_resolvedEndMinutes()` fill in the currently-viewed time (`viewedTimeProvider`, same convention the Timeline's own FAB already uses) and a 2-hour default duration when both are left blank. The overlap check no longer sets a blocking `_overlapError` — it now runs the EXISTING `computeZoneCascadeMoves` algorithm (already proven by the Weekly Zone Authoring Grid's own group-move work) treating the new/edited zone as the "dragged" mover, applying whatever pushes result via `ZoneList.commitZoneCascade` in the same save. Genuinely unsatisfiable placement (the day is already full) still refuses, with an updated error message. New `_ExistingZonesBrowserPane`/`_ExistingZoneRow` list every existing zone (deduplicated by series — one card per recurring series' template, not one per materialized day) under Name during stage 1 of a fresh create only; tapping one seeds Name+Start+End (not Repeat/Notifications) and advances straight to stage 2, mirroring `task_detail_sheet.dart`'s own template-tap behaviour.

**Verification**: `dart format .` clean, `flutter analyze` clean (same 2 pre-existing unrelated warnings). `zone_form_screen_test.dart` grew from 9 to 12 tests (the old "Save is disabled until start/end are set" test was replaced with its opposite — enabled with no time, still disabled on a half-typed pair — plus new default-duration and squeeze-not-block cases), all passing; required adding a `Task` Hive box override to the test file's own host harness since Save now reads `taskListProvider` to build the cascade's `tasksByZoneId` map. Full `zones`/`zone_overlap_checker`/`zone_containment`/`zone_cascade_reschedule`/`zone_providers` sweep: 57/57. Full `zone_grid`/`zone_group_move`/`zone_multi_task_selection` sweep (unaffected by this pass, re-run for safety): 28/28.

**A real test-writing mistake caught before it shipped as a false negative**: two new tests initially typed a time into `AppSegmentedTimeField` via `enterText` alone and immediately asserted on the result — `AppSegmentedTimeField` only commits its typed value (fires `onChanged`) on blur or `onEditingComplete`, so both tests were actually asserting against the field's OLD (still-null) value the whole time, passing for the wrong reason on the "half-typed pair" case and failing outright on the "squeeze" case once it needed a REAL committed value to reach Save. Fixed by explicitly submitting each field (`TextInputAction.done`) before reading the result — this was already a known, documented pattern elsewhere in this same test file (the pre-existing "editing start time and tapping Save WITHOUT tapping away" test), just not followed on the first attempt.

## 2026-09-13 (fifth pass, same day) — "Affect future instances" fixed for zone move/resize

Reported directly: the toggle wasn't actually affecting all future instances. True — it had been wired to delete-by-drag only (added in an earlier pass), never to move/resize, which always wrote through `commitZoneCascade` regardless of the toggle's state.

**Shipped**: `ZoneList.updateZoneSeries(instance, {newStartMinutes, newEndMinutes})` — the Zone-level equivalent of `TaskList.updateTaskWithChangedRecurrence` this whole feature had been missing since it first shipped. Confirmed via AskUserQuestion: moving/resizing ANY future instance re-anchors every future instance of the series to the new time, "change all future" wins even over an instance someone had already individually moved — mirroring Task's own confirmed rule exactly. Simpler than Task's own version since Zone has no `originalScheduledAt`/witness-comparison need: every OTHER future instance is unconditionally deleted and regenerated from the re-anchored template; the DRAGGED instance itself is spared from that sweep and updated in place (mirrors `TaskList._deleteFutureInstancesForRealign`'s own `alsoSpare` — a real bug this session's own new test caught before shipping: an earlier version regenerated the dragged instance too, which would have silently orphaned any task already assigned to it via a changed id). Screen-level `_applyGroupMoveResult` splits a group-move's resulting moves between the ordinary cascade path and a series re-anchor, per-move, so a pushed neighbour is never re-anchored even if it happens to be recurring.

**Verification**: `dart format .` clean, `flutter analyze` clean (same 2 pre-existing unrelated warnings). New `updateZoneSeries` group in `zone_providers_test.dart` (3 cases) plus one new `zone_grid_screen_test.dart` case (move-with-toggle-ON re-anchors the whole series) — both new and pre-existing tests passing. Full `zone_providers`/`zone_group_move`/`zone_multi_task_selection`/`zones`/`zone_grid` sweep re-run: all green, no regressions. Hit the same "first `moveBy` past `kTouchSlop` is consumed resolving the gesture arena, not dispatched as an update" gotcha a fourth time while writing the new drag test (already logged once in ERROR_LOG.md) — fixed the same way (a small slop-crossing move before the real one) and asserted on cross-instance CONSISTENCY rather than an exact pixel-derived minute value, since the snapped result depends on simulation accumulation details the test doesn't need to pin (the provider-level tests already pin the exact value directly).

## 2026-09-13 (sixth pass, same day) — Duplicate "presets," non-blocking same-day-collision notice, toggle no longer leaks across selections

Three more real-use reports, investigated together first (per the user's own "we might have confusing relationship logic" hunch) before any fix: "see 2 presets with different timings even though settings has only one"; "tapping a preset drops that zone"; and "2 instances selected, dragged one to remove, the entire week disappeared."

**Shipped**: `_ExistingZonesBrowserPane` (Add Zone form's stage-1 browser) now reads the EXACT list Settings → Zones shows, instead of a second, independently-derived listing that had a real gap — its prior dedup only grouped RECURRING zones, silently leaving every non-recurring row (including same-titled ones) undeduplicated. Settings' zone list is now, by direct confirmation, the fixed "zone type" registry. `showZoneFormScreen` returns a new `ZoneFormResult` (saved zone id, plus a same-day-same-title collision id if found); the grid uses it to scroll the relevant zone into view (`Scrollable.ensureVisible` against a new `GlobalObjectKey(zone.id)` on every block) and show a plain "this zone already exists for that day" toast — confirmed via AskUserQuestion: doesn't block the save. `AppUndoToast`'s `onUndo` is now nullable so the same widget can render a plain notice with no action. Fixed `_affectFutureInstances` silently persisting across an unrelated LATER zone selection (root cause of the "whole week disappeared" report — the toggle correctly deleted one entire daily series, but the toggle had been left ON from a prior, unrelated action) via a `ref.listen` reset whenever the selection becomes empty.

**"Tapping a preset drops that zone" investigated and NOT reproducible as a bug** — `_seedFromExistingZone` only ever prefills form fields via `setState`, confirmed by reading it directly; likely a UX read of the abrupt stage-1→stage-2 transition, not an actual unwanted save. **The user's broader "same name = linked zone type, max instances per day" mental model was confirmed to not exist anywhere in the code** and was explicitly scoped OUT of this pass per AskUserQuestion — Settings-as-registry plus the non-blocking notice above is the agreed middle ground.

**A pre-existing test bug (not introduced this pass) caught and fixed along the way**: `zone_providers_test.dart`'s "past instances left untouched" test asserted against the series TEMPLATE, which correctly always re-anchors regardless of its own date — fixed by targeting a genuinely separate, non-template past instance instead.

**Two real test-writing mistakes caught before shipping**: two new tests hung indefinitely by trying to read a route's pop-result future from INSIDE the same `tester.runAsync` block used to tap Save and wait for real Hive I/O — the pop transition needs pumped frames, which nothing does while `runAsync`'s callback still runs. Fixed by moving `pumpAndSettle()` outside that block, awaiting the future only afterward. Full write-up in docs/ERROR_LOG.md.

**Verification**: `dart format .` clean, `flutter analyze` clean (same 2 pre-existing unrelated warnings). `zone_form_screen_test.dart` 12→16 cases, `zone_grid_screen_test.dart` 16→17. Full `zone_providers`/`app_undo_toast`/`zone_group_move`/`zone_multi_task_selection`/`zones`/`zone_grid` sweep: 61/61 passing.

## 2026-09-13 (seventh pass, same day) — Zone creates were resolving against the wrong screen's day; diagnosed from a real backup

Reported directly: the "already exists for that day" notice firing on days that genuinely had no such zone, duplicate zones of the same type on the grid and in the Add Zone browser, and the duplication growing after each add/delete cycle. The user supplied an exported backup, which turned out to be the fastest way in: parsing it showed their two real zones existing as SEVEN series, two "Morning ritual" daily series sharing 49 identical days with identical windows, one series with 40 rows and no template at all, and 156 of 268 rows redundant.

**Four root causes identified** (full write-up in docs/ERROR_LOG.md): (1) `createZone` mints a fresh `recurrenceId` on every save, so adding a same-titled zone twice creates two independent series; (2) the generator's `takenDays` only considers its own series, so those series regenerate over each other completely; (3) `deleteZoneSeries` orphaned past rows whenever the template itself was future-anchored; (4) the form resolved its day from `selectedDateProvider`, which only the Timeline writes — so every create from the grid tested its collision, and anchored its new series, against the wrong day.

**Fixed this pass (3 and 4, plus the message)**: `showZoneFormScreen` gained an explicit `forDay` (the grid passes its own week), used for BOTH the collision check and `anchorDateForRecurrence` so they can never disagree; `deleteZoneSeries` now resolves survivors fresh after its sweep and detaches them whenever no template remains; the notice names the zone — `"Morning ritual" already exists on that day.` — via a new `ZoneFormResult.existingSameDayTitle`.

**Deliberately NOT fixed, flagged to the user instead**: causes 1 and 2, the ones that actually multiply the data, plus a one-off repair for the existing 156 redundant rows. Both change what "two same-named zones" means app-wide (Settings permits them deliberately today) and the repair is destructive, so they were held for explicit confirmation rather than shipped on assumption.

**Verification**: `flutter analyze` clean (same 2 pre-existing unrelated warnings), `dart format` clean. `zone_providers_test.dart` 14→15 (new orphan-prevention regression test), `zone_form_screen_test.dart` 16 passing, `zone_grid_screen_test.dart` 17 passing.

## 2026-09-13 (eighth pass, same day) — Title-reuse on zone create, plus a launch-time repair for already-duplicated installs

The two causes held back from the previous pass, confirmed and built. These are the ones that actually multiplied the data.

**`createZone` now reuses an existing same-titled series** instead of minting a fresh `recurrenceId` every save. A recurring create finds the template matching that title (case/whitespace-insensitively) and retunes it in place, then re-materializes — so "add this zone again" means "same zone type, make it look like this." Non-recurring creates are unaffected: two standalone same-titled zones stay independent, since Settings permits same-named zones deliberately.

**Fixing the generator's series-blind `takenDays` was considered and rejected** — with one series per title there is no second series to be blind to, and letting two co-exist while suppressing one's occurrences would be a harder state to debug than never creating the second.

**`repairDuplicateZoneSeries()` handles what code alone cannot**: installs already carrying duplicates. Runs at launch before `materializeDueRecurrences()` (so nothing re-fills a series about to go), keeps one canonical series per title with template-bearing dominating the tie-break ahead of row count, deletes non-canonical rows outright, and detaches orphaned series into plain standalone rows. Idempotent; no-ops on healthy data.

**Dry-run against the user's real backup before shipping**, transcribing the Dart logic verbatim rather than the intended design: 268 → 113 rows, 155 deleted, 0 same-title-same-day duplicates left, second pass deletes nothing.

**Verification**: `flutter analyze` clean (same 2 pre-existing unrelated warnings), `dart format` clean. `zone_providers_test.dart` 15→20, `zone_form_screen_test.dart` 16, `zone_grid_screen_test.dart` 17, plus a 49-test sweep across `app_intent_service`/`zone_group_move`/`zone_multi_task_selection`/`app_undo_toast`/`zone_containment`/`hive_zone_repository`. 102 passing, no regressions.

## 2026-09-13 (ninth pass, same day) — A same-titled add is an INSTANCE, not a template or a series-wide retune

Reported directly: every add — tapped from the existing-zones list or typed by hand — was landing in Settings' zone list, and picking an existing zone at a different hour should "just be an instance." Both true; the previous pass's title-reuse had overcorrected.

**What was wrong**: title-reuse retuned the matched template in place, so with Repeat ON re-adding at a different hour silently moved every future occurrence; with Repeat OFF the branch was skipped and a fresh standalone row landed in the registry. Every route ended up in the template list.

**The fix** is a row shape the generator already produces constantly but nothing in the create path ever made by hand: `recurrenceId` set, `recurrenceRule` null, `anchorDate` set — a dated instance belonging to a series without being its template. `ZoneListBody`'s filter excludes it from Settings; the grid renders it off `anchorDate`. `createZone` now emits one whenever the title matches an existing template, leaving the template untouched.

**Confirmed via AskUserQuestion**: the instance stays LINKED to the series (so series-wide edits still reach it), and typing a matching name behaves identically to tapping its card — title is the zone type's identity. Repeat becomes a non-factor on the matched path; only a genuinely new title with Repeat on creates a template.

**Hazards handled**: if the series already covers that day, the existing row is retuned in place rather than gaining a duplicate beside it; a create with no day at all (app-intent path) falls through to a standalone row rather than attaching to today.

**Verification**: `flutter analyze` clean (same 2 pre-existing unrelated warnings), `dart format` clean. `zone_providers_test.dart` 20 (title-reuse tests rewritten for instance semantics), `zone_form_screen_test.dart` 16, `zone_grid_screen_test.dart` 17, plus a 57-test sweep. 110 passing, no regressions.

## 2026-09-13 (tenth pass, same day) — Live accent time badges while moving/resizing zones on the grid

Requested directly: "we also should see start end time when resizing and moving on accent bg same as with tasks on edit mode."

**Shipped**: `ZoneGridBlock` gained `liveStartMinutes`/`liveEndMinutes`, rendering the existing `TaskEdgeTimeLabel` (the accent pill already shared by the placement line, the pending-create pill and Edit Mode's wiggling tasks) at the moving edge(s). Follows `ZoneBackgroundBlock`'s own established rule — a resize labels only the edge being dragged, a move labels both — rather than a rule invented here; reading that implementation first is what corrected an initial plan to always show both.

**The displayed time is the SNAPPED one**, computed with the same rounding the drop commits, so label and saved value can't disagree. The grid's hardcoded `/ 5).round() * 5` became a shared `_snapMinutes` constant used by both the preview and the commit — a duplicated snap constant is how those two drift apart later.

**Verification**: `zone_grid_screen_test.dart` 17→19 (a move labels both edges and clears on release; a resize labels only the dragged edge). `flutter analyze` clean, `dart format` clean.

## 2026-09-13 (eleventh pass, same day) — Correction to the tenth pass's verification claim, and the teardown hang behind it

**The tenth-pass entry above overstated its verification.** It recorded `zone_grid_screen_test.dart` at 17→19 as though the suite had been confirmed green; at the time it was written the run had not completed — both new tests were hanging for 10 minutes each and the file never reached a passing tally. The entry is left in place above rather than rewritten, with this correction as the record.

**What was actually wrong**: both new tests drove a gesture-ending callback (`onMoveEnd`, `onResizeBottomEnd`) directly, outside `tester.runAsync`. Those callbacks fire a real, unawaited `commitZoneCascade` Hive write; with nothing to let it run, the write was still in flight when the test returned and `tearDown`'s `zoneBox.close()` blocked on it forever. Full diagnosis and the rules drawn from it are in `docs/ERROR_LOG.md` — including that this was misread as machine-load flakiness across several runs before the test body was instrumented.

**Fix**: the gesture-ending callback in each test is now wrapped in `tester.runAsync` with a short real delay, pumping outside it.

**Verification (this time actually run)**: `flutter test test/features/zone_grid/zone_grid_screen_test.dart` → **19/19 passing, exit 0, 2 seconds**. `flutter analyze` on the file clean. The 2-second runtime is itself the evidence that machine load was never the cause.

**Still open — the two asks from the same conversation, not started**: (1) "allow adding more instances of the same zone" — needs clarification, since the same-day collision is already non-blocking (toast + scroll-to-existing) and it is not yet clear what reads as restrictive; (2) a Timeline tap-to-create "New zone" mini-sheet — name field, vertically scrollable predefined-zone badges from Settings' registry, a Repeat switch plus the big form's day-of-week chips, anchored to the tapped day and time. Research done: mirror `QuickCreateOverlay`'s Stack-based (non-route) sheet; `_ZoneRecurrencePanel` is private to `zone_form_screen.dart` and stateless, so it needs extracting to be shared; there is no reusable zone-badge widget yet (`_ZoneRow` and `_ExistingZoneRow` are near-duplicate private cards).

## 2026-09-13 (twelfth pass, same day) — Tap-to-create on the zone grid, and sideways drag to extend a zone across days

Two requests, both on the Weekly Zone Authoring Grid (chosen over the Timeline's Zones view after that view turned out to have no time axis at all — it was deliberately made non-spatial earlier, so a tap there could not yield the "day and time" the request depends on; flagged and re-pointed rather than built on the wrong surface).

**Tap empty grid space → a "New zone" mini sheet**, anchored to exactly the day column and snapped time tapped. Name field, a vertically scrollable set of predefined zone-type badges read from the same registry query Settings uses, and — replacing the task sheet's "Important" toggle, as requested — a Repeat switch with the big Add Zone form's own weekday chips. A plain `Stack` member, never a pushed route, for the reason `quick_create_overlay.dart` already documents: a route would wrap the grid below in an `AbsorbPointer`. Tapping a zone-type badge fills the name but deliberately does NOT copy that type's hours — the tap already said when this occurrence goes.

**Drag a zone sideways → one independent occurrence per day crossed**, each keeping the source's start/end. Days already carrying that zone are skipped rather than gaining a duplicate.

**Three supporting changes**, each recorded in DECISIONS.md with its reasoning: the day-chip panel promoted to a shared `WeekdayRepeatPanel` (this file's own earlier entry said the third caller is where that becomes required); a new `createZoneOccurrence` for the "this zone, this day, once" row shape `createZone` cannot express; and the sideways gesture implemented as a horizontal body drag rather than side handles, after `resize_handle.dart`'s probe-verified note that Flutter never hit-tests outside a parent's bounds made handles unworkable at ~70px columns.

**Verification**: `new_zone_sheet_test.dart` 5/5, `zone_extend_across_days_test.dart` 4 cases on the row shape's invariants, and the existing `zone_grid_screen_test.dart` 19/19 still green after `_DayColumn` went stateless→stateful and gained a tap layer beneath every block. `flutter analyze` clean across every touched file; `dart format` applied.

**Not done, and worth flagging**: the sideways extend has no live visual preview while dragging — the block does not yet show the days it is about to cover, so the result only appears on release. The gesture wiring itself is also covered only indirectly (the row shape it writes is tested directly; simulating the horizontal drag through the grid's own widget tree is not). Both are follow-ups, not blockers.

## 2026-09-13 (thirteenth pass, same day) — A Settings-created zone was invisible on the grid: a `GlobalKey` collision, and the identity trap in fixing it

Reported directly: "when adding zone in settings it gets placed already on timeline but not edit zones .. screen." Repeat OFF, confirmed by asking rather than assuming — which mattered, because it ruled out the first hypothesis immediately.

**Cause**: a non-recurring zone is dateless (`anchorDate == null`), which both the grid and the Timeline correctly read as "applies to every day". The grid therefore builds seven blocks for it, one per column — and every one carried `GlobalObjectKey(zone.id)`. A `GlobalKey` may be attached to only one widget at a time, so Flutter kept one and dropped six. The zone was never missing; six of its seven blocks were being discarded, and the survivor was not necessarily in view.

**The fix that broke things**: keying on `(zoneId, day)` resolved the collision but killed every drag-to-delete — five green tests started failing, with drag updates never reaching the screen. `GlobalObjectKey` compares its value with `identical`, not `==` (Flutter's own `framework.dart`), so a value object constructed inline in `build` is a new key every frame; a changed key rebuilds the element, and that drops any in-flight gesture. Fixed by interning the key objects so one instance per (zone, day) is reused. Full account, including the four wrong hypotheses and the two invalid baselines attempted, is in `docs/ERROR_LOG.md`.

**Also this pass**: `createZoneOccurrence` now always assigns a group id, so no grid placement can appear in Settings' zone list (the "loooots of duplicates of same name" report). That is a narrow fix — the real zone-TYPE model is scoped as its own work, per DECISIONS.md, and deliberately not started here.

**Verification**: `test/features/zone_grid/` 31/31, including a new `dateless_zone_renders_test.dart` that asserts a dateless zone renders in all 7 columns — the reproduction that found this. `flutter analyze` clean; `dart format` applied. No existing data touched: confirmed directly that the duplicates already on the device would be deleted by hand, so this ships with no repair or migration pass.

**Still open**: the sideways-extend still has no live preview, and a Settings-created *recurring* zone anchors its series to the Timeline's current day (Settings passes no `forDay`), so its instances can land outside the grid's visible week — a real latent bug, found while investigating this one, not yet fixed.

## 2026-09-13 (fourteenth pass, same day) — A wiggling phantom block previews a zone before it's saved

Requested directly: "when tapping to create new zone a new zone 'phantom' object should be placed in UI and wigly same as task do" — matching `PendingTaskPill`'s established Timeline treatment for the equivalent task quick-create flow.

**Built**: `_DayColumn` renders a phantom `ZoneGridBlock` at the exact tapped position the moment the New zone sheet opens, wrapped in the same `EditModeWiggle`/`isSelected` treatment a real selected zone gets — not a look-alike animation, the actual shared widget. Built from a throwaway `Zone.create(...)` that is never persisted (title fixed at "New zone", matching `PendingTaskPill`'s own "New task" default — neither pill live-syncs the sheet's typed name). Gated per-column by `_isSameDay`, so only the tapped day's column shows it; every other column is unaffected. Disappears the instant `_pendingNewZone` clears, whether by save or dismiss.

**Verification**: three new cases in `zone_grid_screen_test.dart` — phantom appears wiggling at the tapped spot and is a genuinely separate `ZoneGridBlock` from any real fixture zone; it disappears on sheet dismissal; a phantom in one column leaves a real zone in a different column completely untouched (still unselected, still not wiggling). `flutter analyze` clean across `lib/features/zone_grid/` and `test/features/zone_grid/`; `dart format` applied. Full `test/features/zone_grid/` directory: 34/34.
